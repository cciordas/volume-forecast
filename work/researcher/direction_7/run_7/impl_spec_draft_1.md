...
sig2_eta = var(daily_avg) * (1.0 - a_eta^2)
...
point that typically reduces the number of EM iterations needed.
...
Output: x_filt[1..N][2], Sigma_filt[1..N][2x2],
        x_pred[1..N][2], Sigma_pred[1..N][2x2],
        A_used[2..N][2x2], e[1..N], S[1..N], z_star[1..N]
...
Source: Paper Algorithm 1 (lines 2-6), Equations 4-5. Robust modification from Section
3.1, Equations 29-34. The Joseph form covariance update (I-KC) Sigma (I-KC)^T + K r K^T
is algebraically equivalent to the paper's Sigma - K C Sigma (Algorithm 1, line 6) but
guarantees symmetry and positive semi-definiteness; researcher inference (standard
Kalman filter practice, not discussed in paper). Soft-thresholding from Equations 33-34.
The equivalence threshold = lambda*S/2 = lambda/(2*W) holds because W = 1/S (Equation
30). Missing observation handling is researcher inference (paper excludes zero-volume
bins per Section 4.1).
...
Source: Paper Algorithm 2, Equations 10-11. The analytic 2x2 inverse with epsilon
regularization is researcher inference (standard numerical practice for small matrices).
...
a_eta = numer_eta / denom_eta           # Eq A.34
...
a_mu = numer_mu / denom_mu             # Eq A.35
...
```
Input: e[1..N], S[1..N], observed[1..N]
Output: log_likelihood (scalar)

# Innovation-form log-likelihood
N_obs = count of tau where observed[tau]
log_lik = -0.5 * sum(ln(S[tau]) + e[tau]^2 / S[tau] for tau where observed[tau])
         - (N_obs / 2.0) * ln(2 * pi)
```

Source: Researcher inference (standard Kalman filter innovation log-likelihood; Shumway
and Stoffer, 1982). The paper does not specify the convergence criterion. For the robust
variant, e[tau] in this formula is the raw innovation (before subtracting z_star), and
the log-likelihood serves as a convergence heuristic rather than the exact observed-data
log-likelihood (the Lasso penalty on z makes the robust EM not a standard EM, so
monotonic increase is not strictly guaranteed). A parameter-change fallback criterion
max(|theta_new - theta_old| / (|theta_old| + 1e-10)) < tol can be used as backup.
...
#### Step 13: Rolling-Window Calibration with Cross-Validation
...
| x_filt, x_pred, x_smooth | float64 | (N, 2) | State vectors |
| Sigma_filt, Sigma_pred, Sigma_smooth | float64 | (N, 2, 2) | State covariance matrices |
...
| z_star | float64 | (N,) | Outlier corrections (0 for standard) |
...
with lowest average validation MAPE. See Step 13 for full procedure.
...
| CMEM | 7.01 | 7.71 |
| Rolling Mean | 7.48 | 7.48 |

Source: Paper Table 4, Equation 42. Robust KF improves over RM by ~15% (dynamic) and
over CMEM by ~9%. Cross-check: (7.48 - 6.38) / 7.48 = 14.7% ~ 15%.
...
2. **Log-likelihood monotonicity:** For standard model, innovation log-likelihood must
   be non-decreasing across EM iterations. A decrease signals an implementation bug.
   For robust model, monotonicity is not strictly guaranteed (see Step 6 notes).
...
| Kalman filter algorithm | Algorithm 1, lines 2-6 | |
...
| M-step: Sigma_1 = Sigma_smooth[1] | Appendix A, Equation A.33 | Researcher inference: equivalent form avoiding cancellation |
| M-step: a_eta | Appendix A, Equation A.34 | |
| M-step: a_mu | Appendix A, Equation A.35 | |
| M-step: sig2_eta | Appendix A, Equation A.36 | Uses a_eta^(j+1) |
| M-step: sig2_mu | Appendix A, Equation A.37 | Uses a_mu^(j+1) |
| M-step: r | Appendix A, Equation A.38 | Uses phi^(j+1); compact form via P decomposition |
| M-step: phi | Appendix A, Equation A.39 | |
...
| MAPE formula | Section 3.3, Equation 37 | |
...
| Cross-validation procedure | Section 4.1 | Grid values are researcher inference |
...
| Static VWAP TE benchmarks | Table 4 | Average over 30 securities |
