...
Repeat until convergence:
    ...
    Run Kalman filter (Algorithm 1) with theta^(j) to get:
        ...
        S_tau for tau = 1..N  (innovation variances, stored from filter)
    ...
    LL^(j) = -(N/2) * log(2*pi) - (1/2) * sum_{tau=1}^{N} [log(S_tau) + innovation_tau^2 / S_tau]
    # where innovation_tau and S_tau are stored from the Kalman filter E-step run
...
- Relative change in innovation-based log-likelihood < epsilon (default epsilon = 1e-6):
  ...
  where LL^(j) = -(N/2)*log(2*pi) - (1/2)*sum_tau [log(S_tau) + innovation_tau^2 / S_tau]
