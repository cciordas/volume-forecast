...
Repeat until convergence:
    ...
    # Note: the a^eta, a^mu, (sigma^eta)^2, and (sigma^mu)^2 updates above use smoothed
    # sufficient statistics (P_tau, P_{tau,tau-1}), which are defined for all tau regardless
    # of whether y_tau was observed. The smoother always runs because the prediction step
    # propagates state at every tau. Only sums involving raw observations y_tau (phi, r,
    # and log-likelihood) require restriction to O.
