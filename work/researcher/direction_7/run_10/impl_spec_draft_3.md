...
Set j = 0
Set max_iterations = 100      # safety bound (typical convergence in 5-20 iterations)
...
where e_tau = y[tau] - C * x_hat[tau|tau-1] - phi_tau is the innovation and
...
Gaussian state-space model. Missing bins are excluded from the sum. Reference: Paper
Appendix A.1, Eq (A.8); standard Kalman filter theory.
...
account for the inferred outlier values z_star_tau:
...
For bin i = 1, 2, ..., I-1:
  ...
  # At bin i (unified index tau_i), produce forecasts for bins i, i+1, ..., I.
  # Bin i itself is one-step-ahead (h=0); bin i+h is h additional steps ahead.
  # Use the multi-step prediction recursion from Step 2:
  #   Sigma[tau_i + h | tau_i] for h = 0, 1, ..., I-i
  # where Sigma[tau_i + 0 | tau_i] = Sigma[tau_i | tau_i - 1] (the prediction covariance).
  For h = 0, 1, ..., I-i:
    # h=0: V_{i} = C * Sigma[tau_i | tau_i - 1] * C^T + r   (one-step-ahead)
    # h>0: V_{i+h} = C * Sigma[tau_i + h | tau_i] * C^T + r (multi-step, variance grows with h)
    V_{i+h} = C * Sigma[tau_i + h | tau_i] * C^T + r
...
| max_iterations | Maximum EM iterations (safety bound) | 100 (Paper Fig 4 shows convergence in ~5-10 iterations, so 100 is very conservative) | Low -- only relevant if EM fails to converge due to numerical issues. | [50, 500] |
...
2. Rolling window re-estimation:
   ...
   b. Run EM to convergence (typically 5-20 iterations, max 100)
