...
Definitions -- Sufficient Statistics:
  ...
  Let D_start = {tau : tau = k*I+1 for k=1,2,...} be the set of first-bin-of-day indices.
...
Procedure:
  ...
  Repeat:
    ...
    -- Compute log-likelihood for convergence check (Equation A.8):
    log_lik^(j) = - sum_{tau=1}^{N} (y[tau] - phi[tau] - C * x_hat_filt[tau])^2 / (2*r)
                  - (N/2) * ln(r)
                  - sum_{tau=2}^{N} (mu_hat[tau] - a_mu * mu_hat[tau-1])^2 / (2*sigma_mu^2)
                  - ((N-1)/2) * ln(sigma_mu^2)
                  - sum_{tau in D_start} (eta_hat[tau] - a_eta * eta_hat[tau-1])^2 / (2*sigma_eta^2)
                  - ((T-1)/2) * ln(sigma_eta^2)
                  - (1/2) * (x_hat[1] - pi_1)^T * Sigma_1^{-1} * (x_hat[1] - pi_1)
                  - (1/2) * ln(|Sigma_1|)
                  - ((2N+T)/2) * ln(2*pi)

    where x_hat_filt[tau] are the smoothed states, mu_hat and eta_hat are their
    components. In practice, since EM guarantees monotonic increase, tracking the
    relative change |log_lik^(j) - log_lik^(j-1)| / |log_lik^(j-1)| is sufficient.
    Alternatively, monitoring the parameter change norm
    ||theta^(j+1) - theta^(j)|| / ||theta^(j)|| works as a convergence criterion.

    (Paper: Appendix A.1, Equation A.8)
...
Here volume_hat_dynamic[i] is the one-step-ahead FORECAST for bin i, made BEFORE
bin i is observed (i.e., the predicted volume, not the actual). At bin i, the weight
uses the forecast value, not the realized observation. For bins j > i, the forecasts
are the current best predictions given information up to bin i-1 (or i, depending on
when the dynamic update runs). The paper's Equation 41 uses the notation
volume_{t,i}^{(d)} to denote this dynamic prediction.
...
| Log-likelihood formula | Chen et al. 2016, Appendix A.1, Equation A.8 |
