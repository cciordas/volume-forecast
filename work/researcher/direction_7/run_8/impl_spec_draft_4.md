...
REPEAT until convergence (or max_iterations):
    ...
    // Note: since P[1] = Sigma[1|N] + x_hat[1|N]*x_hat[1|N]^T, this simplifies to Sigma[1|N].
...
DEFINE:
    ...
    // NOTE on day boundary convention: The transition from tau to tau+1
    // crosses a day boundary when tau is the LAST bin of a day (tau mod I == 0),
    // equivalently when tau+1 is the FIRST bin of a new day (tau+1 in D, where
    // D = {I+1, 2I+1, ...}). Both conditions identify the same transition.
    // The set D marks the destination bin of the boundary crossing; A_tau and
    // Q_tau use the source bin tau as the conditioning index.
...
FOR tau = 1, 2, ..., N:
    ...
    K[tau] = Sigma[tau|tau-1] * C^T / S[tau]                      // Eq 7
    
    // Corrected state estimate (posterior):
    x_hat[tau|tau] = x_hat[tau|tau-1] + K[tau] * e[tau]           // Eq 8
    
    // Corrected covariance (posterior):
    Sigma[tau|tau] = Sigma[tau|tau-1] - K[tau] * S[tau] * K[tau]^T
                   = (I_2 - K[tau] * C) * Sigma[tau|tau-1]        // Eq 8 (covariance form)
    
    // --- PREDICTION STEP (project state forward to tau+1) ---
    x_hat[tau+1|tau] = A_tau * x_hat[tau|tau]                     // Eq 7 (prediction)
    Sigma[tau+1|tau] = A_tau * Sigma[tau|tau] * A_tau^T + Q_tau   // Eq 7 (covariance prediction)
    
    // NOTE on loop ordering vs Algorithm 1: This spec iterates over the
    // observation index tau directly, performing forecast -> correct -> predict
    // at each tau. Algorithm 1 in the paper (page 4) orders the operations as
    // predict (lines 2-3), then correct (lines 4-6), iterating over tau+1.
    // The equations are mathematically identical (just a relabeling of the
    // loop index); only the loop structure differs. Citations above use
    // equation numbers from Section 2.2, not Algorithm 1 line numbers, to
    // avoid confusion from this reordering.
...
**Robust EM data flow clarification:** During robust EM, the E-step consists of two sub-steps: (a) the robust Kalman filter forward pass (Step 5 above), which computes and STORES z_star[tau] for each tau alongside the usual filter outputs; followed by (b) the standard RTS smoother (Step 4, Algorithm 2 -- unchanged from the non-robust case). The M-step then uses both the smoother outputs (x_hat[tau|N], Sigma[tau|N], etc.) AND the stored z_star values. The smoother itself is NOT modified for the robust extension; only the filter's correction step and the M-step updates for r and phi change.
...
**VWAP tracking error:**
    ...
    VWAP_TE (bps) = (10000/D) * sum_{t=1}^{D} |VWAP_t - replicated_VWAP_t| / VWAP_t
...
where price_{t,i} is the last recorded transaction price in bin i of day t. The raw formula |VWAP - repl_VWAP| / VWAP produces a fraction (e.g., 0.000638); multiply by 10,000 to convert to basis points (1 bps = 0.01%). The paper's Table 4 reports values in basis points.
...
9. **Observation noise variance r near zero:** The M-step update for r (Eq A.38) computes r as a sample average of expected squared residuals, which is theoretically always positive. In practice, if the model overfits or with finite-precision arithmetic, r could approach zero or become slightly negative. If this occurs, clip r to a small positive floor (e.g., 1e-10). This should not happen with reasonable volume data but is a defensive safeguard.
...
| Kalman filter algorithm | chen_feng_palomar_2016, Section 2.2, Equations 7-8, Algorithm 1, page 4 |
