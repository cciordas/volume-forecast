...
ROBUST_KALMAN_FILTER(y[1..N], theta, lambda, I):
    ...
    # One-step-ahead predictions (same as standard filter)
    y_hat[tau] = C @ x_hat_pred[tau] + phi[i]   # for each tau
...
ROBUST_EM(y[1..N], I, theta_init, lambda, max_iter, tol):
    ...
    repeat:
        ...
        x_hat_filt, Sigma_filt, x_hat_pred, Sigma_pred, A, K, S, e, z_star, _ = ROBUST_KALMAN_FILTER(y, theta, lambda, I)
        # Note: y_hat (last return) is not needed during EM iteration, only for prediction
        ...
        # log-likelihood for the robust model. Because z_star is a deterministic function
        # of e (via soft-thresholding), e_corrected has a truncated distribution and does
        # not follow the standard Gaussian innovation distribution. EM convergence is
        # guaranteed by the E-step/M-step structure regardless of this monitor.
        # (Researcher inference -- the paper does not discuss log-likelihood computation
        # for the robust EM.)
...
CALIBRATE(y_all, I, N_train, N_val, lambda_grid):
    ...
    for each out-of-sample day d:
        ...
        _, _, _, _, _, _, _, _, _, y_hat = ROBUST_KALMAN_FILTER(y_day_d, theta, best_lambda, I)
        forecasts[d] = y_hat
...
| Robust filter y_hat return | Researcher inference (parallel interface with standard filter; needed for prediction) |
| Robust EM log-likelihood is convergence heuristic | Researcher inference (z* is deterministic function of e; see comment in robust EM) |
