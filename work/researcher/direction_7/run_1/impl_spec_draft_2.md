...
KALMAN_FILTER(y[1..N], theta, I):
    ...
    for tau = 1, 2, ..., N:
        ...
        Sigma[tau|tau] = IminusKC @ Sigma[tau|tau-1] @ IminusKC^T + K[tau] * r * K[tau]^T
...
ROBUST_KALMAN_FILTER(y[1..N], theta, lambda, I):
    ...
    return x_hat, Sigma, K, S, e, z_star
```
...
ROBUST_EM(y[1..N], I, theta_init, lambda, max_iter, tol):
    ...
    repeat:
        ...
        x_hat_filt, Sigma_filt, K, S, e, z_star = ROBUST_KALMAN_FILTER(y, theta, lambda, I)
        ...
        # Convergence check (same as standard EM)
        if j > 1:
            delta = |log_lik - log_lik_prev|
        log_lik_prev = log_lik
...
CALIBRATE(y_all, I, N_train, N_val, lambda_grid):
    ...
    for each out-of-sample day d:
        ...
        theta = EM_ALGORITHM(y_train, I, theta_init_d, max_iter=100, tol=1e-6)
        theta_prev = theta
