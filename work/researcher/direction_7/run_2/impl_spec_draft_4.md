...
15-minute). Zero-volume bins must be handled as unobserved (see Edge Cases).
...
FUNCTION kalman_filter(y[1..N], theta):
    ...
    FOR tau = 1 TO N:
        ...
        A_stored[tau] = A if tau > 1 else I_2  # transition used at this step
            # Note: A_stored[1] = I_2 is a placeholder; the smoother loop
            # (tau = N-1 down to 1) accesses A_stored[tau+1], so A_stored[1]
            # is never read. Stored for index consistency only.

        # Log-volume forecast (one-step-ahead, before correction):
        y_hat[tau] = C @ x_pred + phi_tau
    END FOR

    RETURN x_filtered, Sigma_filtered, Sigma_predicted, A_stored, y_hat
END FUNCTION
```
...
FUNCTION kalman_smoother(x_filtered[1..N], Sigma_filtered[1..N],
                          Sigma_predicted[1..N], A_stored[1..N], theta):
    ...
    #
    # Non-recursive formula (Shumway & Stoffer 1982, Property 6.3):
    #   Sigma_{tau, tau-1|N} = Sigma_{tau|N} @ L_{tau-1}^T
    #
    # This is mathematically equivalent to the paper's recursive Eq A.20
    # (which uses FILTERED covariance Sigma_{tau|tau}). The equivalence
    # follows from substituting the smoother gain definition and the
    # smoother covariance update into A.20 (see derivation below).
    #
    # Advantages over the recursive A.20 formula:
    #   - No recursion (each tau is independent)
    #   - No separate initialization step (A.21 is not needed)
    #   - Uses only quantities already computed by the smoother
    #   - Eliminates the filtered/smoothed confusion risk
    #
    # Derivation of equivalence:
    # Starting from A.20 with filtered covariance:
    #   Sigma_{tau,tau-1|N} = Sigma_{tau|tau} L_{tau-1}^T
    #       + L_tau (Sigma_{tau+1,tau|N} - A_tau Sigma_{tau|tau}) L_{tau-1}^T
    # Substituting Sigma_{tau+1,tau|N} = Sigma_{tau+1|N} L_tau^T (the S&S
    # formula at one step later) and using A_tau Sigma_{tau|tau} =
    # Sigma_{tau+1|tau} L_tau^T (from the definition of L_tau), the second
    # term becomes L_tau (Sigma_{tau+1|N} - Sigma_{tau+1|tau}) L_tau^T L_{tau-1}^T.
    # By the smoother covariance update, Sigma_{tau|tau} +
    # L_tau (Sigma_{tau+1|N} - Sigma_{tau+1|tau}) L_tau^T = Sigma_{tau|N}.
    # Therefore: Sigma_{tau,tau-1|N} = Sigma_{tau|N} L_{tau-1}^T.

    FOR tau = N DOWN TO 2:
        Sigma_cross[tau] = Sigma_smooth[tau] @ L[tau-1]^T
    END FOR

    RETURN x_smooth, Sigma_smooth, Sigma_cross, L
END FUNCTION
```

(Paper, Section 2.3.1, Algorithm 2, Equations 10-11; Appendix A.2, Equations
A.18-A.22; Shumway & Stoffer 1982)

**Note on cross-covariance formula:** The paper's Equation A.20 gives a
recursive formula using FILTERED covariance Sigma_{tau|tau} (not smoothed).
Draft 2 incorrectly changed this to smoothed covariance based on an erroneous
critique -- using smoothed covariance in A.20 produces incorrect results
because A_tau Sigma_{tau|N} != Sigma_{tau+1|tau} L_tau^T when Sigma_{tau|N} !=
Sigma_{tau|tau}. This draft replaces the recursive formula entirely with the
simpler non-recursive Shumway & Stoffer (1982) formula, which uses smoothed
covariance correctly and produces identical results.
...
FUNCTION em_calibrate(y[1..N], I, max_iter, tol):
    ...
    FOR j = 0, 1, 2, ... UNTIL convergence:
        ...
        x_filt, Sigma_filt, Sigma_pred, A_stored, _ = kalman_filter(y, theta)

        # Run backward RTS smoother
        x_smooth, Sigma_smooth, Sigma_cross, L = kalman_smoother(
            x_filt, Sigma_filt, Sigma_pred, A_stored, theta)
        ...
        Sigma_1 = Sigma_smooth[1]                                  # (Eq A.33)
        # Note: the paper writes Sigma_1 = P[1] - x_hat[1] @ x_hat[1]^T,
        # which equals Sigma_smooth[1] by definition of P[1] (Eq A.19:
        # P_tau = Sigma_smooth[tau] + x_hat[tau] @ x_hat[tau]^T). Using
        # Sigma_smooth[1] directly avoids the numerically unstable subtraction
        # of two positive semi-definite matrices of similar magnitude.
        ...
        Q_curr = compute_Q(theta, x_hat, P, P_cross, y, phi, Sigma_smooth)
...
where the E terms are defined in Equations A.11-A.14:

    E_1 = (1 / (2r)) * SUM_{tau=1}^{N}
        [ y_tau^2 + C @ P[tau] @ C^T - 2 * y_tau * C @ x_hat[tau]
          + phi_tau^2 - 2 * y_tau * phi_tau + 2 * phi_tau * C @ x_hat[tau] ]

    Expanding C @ P[tau] @ C^T with C = [1,1]:
        C @ P[tau] @ C^T = P[tau]^(1,1) + 2 * P[tau]^(1,2) + P[tau]^(2,2)

    Equivalently (using P = Sigma_smooth + x_hat @ x_hat^T):
        E_1 = (1 / (2r)) * SUM_{tau=1}^{N}
            [ (y_tau - phi_tau - C @ x_hat[tau])^2
              + C @ Sigma_smooth[tau] @ C^T ]

    (Paper, Appendix A.1, Equation A.11)

    E_2 = (1 / (2 * sigma_mu_sq)) * SUM_{tau=2}^{N}
        [ P[tau]^(2,2) + a_mu^2 * P[tau-1]^(2,2)
          - 2 * a_mu * P_cross[tau]^(2,2) ]

    (Paper, Appendix A.1, Equation A.12)

    E_3 = (1 / (2 * sigma_eta_sq)) * SUM_{tau in D'}
        [ P[tau]^(1,1) + a_eta^2 * P[tau-1]^(1,1)
          - 2 * a_eta * P_cross[tau]^(1,1) ]
        where D' = {kI+1 : k = 1, ..., T-1}

    (Paper, Appendix A.1, Equation A.13)

    E_4 = (1/2) * (x_hat[1] - pi_1)^T @ inv(Sigma_1) @ (x_hat[1] - pi_1)
        + (1/2) * tr(inv(Sigma_1) @ Sigma_smooth[1])

    (Paper, Appendix A.1, Equation A.14)

**Simplification after M-step:** After applying the M-step updates, the E terms
simplify because the M-step sets each parameter to the value that zeroes the
corresponding derivative. Specifically:

    E_1 = N/2  (since r_new = (1/N) * [inner sum of E_1 * 2r])
    E_2 = (N-1)/2
    E_3 = (T-1)/2
    E_4 = (1/2) * tr(inv(Sigma_1) @ Sigma_smooth[1])
        = 1    (since Sigma_1 = Sigma_smooth[1] after M-step, so tr(I_2) = 2,
                but the full E_4 also includes the mean term which is zero
                since pi_1 = x_hat[1]; so E_4 = (1/2) * tr(I_2) = 1)

So the post-M-step Q reduces to:
...
This means convergence can be monitored by checking the log-determinant terms
alone, since the constant terms do not change between iterations.
(Researcher inference: the simplification follows directly from the M-step
optimality conditions; not stated in the paper.)

**Alternative convergence criterion:** Instead of computing Q, monitor the
relative change in parameter values: max_i |theta_i^{j+1} - theta_i^{j}| /
|theta_i^{j}| < tol. This avoids computing Q entirely and is simpler to
implement. Both criteria are valid; Q monitoring has the advantage that a
decrease signals an implementation bug. (Researcher inference)
...
FUNCTION robust_kalman_filter(y[1..N], theta, lambda):
    ...
    RETURN x_filtered, Sigma_filtered, Sigma_predicted, A_stored, y_hat, z_detected
END FUNCTION
```
...
FUNCTION compute_vwap_weights_dynamic(vol_hat_dynamic[1..I]):
    ...
    #   collected after running the filter through the full day. At each bin i,
    #   the filter was corrected with y[i-1] and predicted one step to bin i.
    #   All I forecasts are available before calling this function.
    #   This matches the paper's evaluation (Section 4.3, Table 4).
    #
    # Production mode: at each bin i, the filter state is corrected through
    #   bin i-1, then multi-step-ahead forecasts are computed for bins i..I.
    #   Only the remaining bins [i..I] are passed to this function (or
    #   equivalently, the full vector is passed with bins 1..i-1 set to their
    #   actual volumes). The cumulative_weight for bins 1..i-1 must be tracked
    #   externally across calls, since their actual execution weights (not
    #   forecast-based weights) are already committed.
    #
    # The mathematical formula (Eq 41) is identical in both modes. The
    # difference is in what vol_hat_dynamic contains: one-step-ahead forecasts
    # collected retrospectively (backtesting) vs. multi-step-ahead forecasts
    # from the current state (production).
...
Log transform  -->  y_{t,i} = log(volume_{t,i})
    ...
    |                (If volume == 0: mark bin as unobserved, do NOT remove from series)
    v
+---------------------------------------------------+
...
1. **Data preparation:**
   ...
   e. Handle zero-volume bins: mark as unobserved (do NOT remove from the
      time series). The tau index must always advance through all I bins per
      day, even for unobserved bins, because the day-boundary detection
      (tau mod I == 0) requires the full I-bin day structure. Removing
      individual bins from the time series will corrupt the transition
      matrix logic. For the Kalman filter, unobserved bins skip the
      correction step (use the "IF y[tau] is observed: ... ELSE: propagate
      prediction" branch in Algorithm 1). For training data, an alternative
      is to exclude entire days that contain any zero-volume bin, which
      preserves the N = T * I structure without requiring missing-data
      handling. (Paper, Section 4.1: "the model assumes non-zero volumes")
...
1. **Zero-volume bins:** log(0) is undefined. The model cannot handle bins with
   zero traded volume. For training: either exclude entire days containing
   zero-volume bins (preserves N = T * I structure), or mark zero-volume bins
   as unobserved and skip the Kalman correction step. For online prediction:
   always mark as unobserved and skip correction (never remove bins from the
   tau sequence). The tau index must advance through all I bins per day even
   for unobserved bins, because the day-boundary detection (tau mod I == 0)
   requires the full I-bin day structure. Removing individual bins corrupts
   the transition matrix logic.
   (Paper, Section 4.1: "the model assumes non-zero volumes")
...
| Cross-covariance (non-recursive formula) | Shumway & Stoffer (1982); equivalent to Paper, Appendix A.2, Eq A.20 with filtered covariance |
| Cross-covariance equivalence derivation | Researcher inference (algebraic proof using smoother gain definition and smoother covariance update) |
...
| M-step: pi_1 | Paper, Appendix A.3, Equation A.32 |
| M-step: Sigma_1 = Sigma_smooth[1] | Paper, Appendix A.3, Equation A.33 (simplified via A.19) |
...
| Q function E_1 term | Paper, Appendix A.1, Equation A.11 |
| Q function E_2 term | Paper, Appendix A.1, Equation A.12 |
| Q function E_3 term | Paper, Appendix A.1, Equation A.13 |
| Q function E_4 term | Paper, Appendix A.1, Equation A.14 |
| Post-M-step Q simplification | Researcher inference (follows from M-step optimality conditions) |
...
| Zero-volume bin handling (mark unobserved, preserve tau index) | Researcher inference (follows from day-boundary detection logic; paper assumes non-zero volumes) |
...
| Alternative convergence criterion (parameter change) | Researcher inference |

## Revision History

### Draft 4 changes (addressing Critique 3)

**No major issues.**

**Minor issues addressed:**
- m1: Added detailed USAGE MODES comment to compute_vwap_weights_dynamic
  distinguishing backtesting mode (one-step-ahead forecasts collected
  retrospectively) from production mode (multi-step-ahead forecasts from
  current state, with cumulative_weight tracked externally). The mathematical
  formula (Eq 41) is identical in both modes; only the contents of
  vol_hat_dynamic differ.
- m2: Added comment to A_stored[1] = I_2 in Algorithm 1 clarifying it is a
  placeholder for index consistency and is never read by the smoother (which
  accesses A_stored[tau+1] for tau = N-1 down to 1).

### Draft 3 changes (addressing Critique 2)

**Major issues addressed:**
- M1: Cross-covariance recursion reverted from (incorrect) smoothed covariance
  back to the correct formulation. Replaced the paper's recursive Eq A.20
  entirely with the simpler non-recursive Shumway & Stoffer (1982) formula:
  Sigma_{tau,tau-1|N} = Sigma_{tau|N} @ L_{tau-1}^T. This eliminates the
  recursion, removes the initialization step (A.21), removes the
  filtered/smoothed confusion risk, and uses only quantities already computed
  by the smoother. Added derivation proving equivalence with A.20. Removed
  K_stored from Algorithm 1 output (no longer needed by smoother). Updated
  Algorithm 2 signature and smoother to return L (smoother gains).

**Minor issues addressed:**
- m1: Added explicit E_1 through E_4 formulas from Equations A.11-A.14,
  including the simplification showing that E_1 = N/2, E_2 = (N-1)/2,
  E_3 = (T-1)/2, E_4 = 1 after the M-step, reducing Q to log-determinant
  terms only. Added alternative parameter-change convergence criterion.
- m2: Clarified zero-volume bin handling throughout the spec. Data preparation
  step now explicitly states: mark as unobserved (do NOT remove from series).
  Added explanation of why tau index must advance through all I bins per day.
  Distinguished training options (exclude entire days vs. mark bins unobserved)
  from online prediction (always mark unobserved). Updated Data Flow diagram.
  Updated Edge Case 1 with consistent guidance.
- m3: Replaced Sigma_1 = P[1] - x_hat[1] @ x_hat[1]^T with
  Sigma_1 = Sigma_smooth[1] in Algorithm 3. Added comment explaining the
  mathematical equivalence (via P definition A.19) and numerical advantage.
...
**Minor issues addressed:**
- m1: Fixed cross-covariance recursion in Algorithm 2 to use Sigma_smooth (smoothed covariance) instead of Sigma_filtered. (NOTE: this was INCORRECT -- critique 1's m1 was wrong. Reverted in draft 3.)
