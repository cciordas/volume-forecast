...
**Proof sketch:** From the RTS backward recursion:

  x_smooth[tau-1] = x_filt[tau-1] + L[tau-1] @ (x_smooth[tau] - x_pred[tau])

Since x_filt[tau-1] is determined by observations up to tau-1 and is therefore
uncorrelated with the smoother correction (x_smooth[tau] - x_pred[tau])
conditioned on filtering, the cross-covariance reduces to:

  Cov(x_tau, x_{tau-1}) = Cov(x_tau, L[tau-1] @ x_smooth[tau])
                         = Sigma_smooth[tau] @ L[tau-1]^T

The key step: Cov(x_tau, L[tau-1] @ x_smooth[tau]) = Cov(x_smooth[tau],
x_smooth[tau]) @ L[tau-1]^T = Sigma_smooth[tau] @ L[tau-1]^T, because
x_smooth[tau-1]'s dependence on x_smooth[tau] is entirely through the
L[tau-1] @ x_smooth[tau] term (the x_filt[tau-1] term being conditionally
independent).
...
INPUT: theta, x_filt_prev (filtered state at bin tau-1),
       ...
       lambda, is_day_boundary (boolean)
       # In the EM loop context, is_day_boundary = ((tau - 1) mod I == 0).
       # In the dynamic VWAP context, the caller supplies this explicitly
       # (true for i=1, false otherwise) since there is no global tau index.
...
INPUT: theta, x_filt[N], Sigma_filt[N], I, total order quantity Q_total,
       live volume observations y_live[1..I], lambda
OUTPUT: execute[1..I] (shares executed per bin)

cumulative_executed = 0.0

# Initialize state: end of training day (last filtered state)
x_filt_current = x_filt[N]
Sigma_filt_current = Sigma_filt[N]

for i = 1 to I - 1:
    # Determine if this step crosses a day boundary
    # Only the first bin (i=1) requires a day-boundary transition
    # (from end of training day to start of new day)
    is_boundary = (i == 1)

    # Forecast current bin from pre-correction state, then correct
    # Step 9 is called with explicit is_day_boundary parameter
    y_hat_i, volume_hat_i, x_filt_updated, Sigma_filt_updated =
        dynamic_predict(theta, x_filt_current, Sigma_filt_current,
                        ...
                        is_day_boundary=is_boundary)                  # Step 9

    # Forecast remaining bins i+1..I from PRE-CORRECTION state
    # This matches the paper's Eq 41 where all volume forecasts in the
    # weight ratio are conditioned on the same information set (observations
    # up to bin i-1). The pre-correction state x_filt_current does not yet
    # incorporate y_live[i].
    #
    # Implementation note: we need x_pred (the predicted state at bin i
    # before correction). Since dynamic_predict internally computes x_pred
    # but returns x_filt_updated, we reconstruct x_pred and Sigma_pred:
    if is_boundary:
        A = [[a_eta, 0], [0, a_mu]]
        Q = [[sigma_eta_sq, 0], [0, sigma_mu_sq]]
    else:
        A = [[1, 0], [0, a_mu]]
        Q = [[0, 0], [0, sigma_mu_sq]]
    x_pred_i = A @ x_filt_current
    Sigma_pred_i = A @ Sigma_filt_current @ A^T + Q

    # volume_hat_i was already computed from x_pred_i inside Step 9
    # (y_hat_i = C @ x_pred_i + phi[i], volume_hat_i = exp(y_hat_i))
    # Now forecast remaining bins from the same pre-correction state
    volume_hat_remaining[i+1..I] =
        multi_step_ahead(theta, x_pred_i, Sigma_pred_i, i, I)        # Step 10

    # Total forecasted volume for bins i..I (all from same info set)
    vol_remaining = volume_hat_i + sum(volume_hat_remaining[i+1..I])

    # Weight for bin i: proportion of remaining to execute (Paper, Eq 41)
    ...
    # Update state for next iteration (post-correction)
...
**VWAP tracking error formulas** (Paper, Section 4.3, Equations 39, 42):

    # Actual market VWAP for day t (Paper, Eq 39):
    VWAP_t = sum_{i=1}^{I}(volume_{t,i} * price_{t,i}) / sum_{i=1}^{I}(volume_{t,i})

    # Replicated VWAP from the strategy:
    replicated_VWAP_t = sum_{i=1}^{I}(w_{t,i} * price_{t,i})
    # where w_{t,i} are the static or dynamic VWAP weights

    # Tracking error (Paper, Eq 42):
    VWAP_TE = (1/D) * sum_{t=1}^{D} |VWAP_t - replicated_VWAP_t| / VWAP_t

where D is the number of out-of-sample days, and price_{t,i} is the last
transaction price in bin i of day t (Paper, Section 4.1). Note that the replicated
VWAP formula assumes the strategy executes at the bin's closing price, which is
a standard backtesting assumption.

**VWAP tracking error benchmarks** (Paper, Table 4, basis points):
...
| VWAP tracking error formula   | Section 4.3, Equations 39, 42                        |
| Actual VWAP formula           | Section 4.3, Equation 39                             |
| Dynamic VWAP info-set fix     | Section 4.3, Equation 41 (pre-correction consistency)|
| Dynamic VWAP boundary param   | Researcher inference (explicit is_day_boundary flag)  |
