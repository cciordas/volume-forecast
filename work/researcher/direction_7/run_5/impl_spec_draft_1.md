...
OUTPUT: x_filt[1..N], Sigma_filt[1..N], x_pred[1..N], Sigma_pred[1..N],
        A_used[2..N], e[1..N], S[1..N]

# Storage note: A_used[tau] stores the transition matrix used to produce the
# prediction at tau (i.e., A[tau-1] in the paper's notation, applied at step tau-1).
# This is needed by the smoother at the corresponding backward step.
...
OUTPUT: x_smooth[1..N], Sigma_smooth[1..N], Sigma_cross[2..N]
...
for tau = 2 to N:
    # Sigma_cross[tau] = E[x_tau * x_{tau-1}^T | all data] - covariance part
    # Using Shumway & Stoffer (1982) formula:
    #   Sigma_cross[tau] = Sigma_smooth[tau] @ L[tau-1]^T
    # where L[tau-1] is the smoother gain computed above.
    # We need L[tau-1], so we must store it during the backward pass above.
    # Alternatively, recompute: L[tau-1] = Sigma_filt[tau-1] @ A_used[tau]^T @ inv(Sigma_pred[tau])
...
The backward recursion for x and Sigma is standard RTS. The cross-covariance
Sigma_cross uses the non-recursive formula from Shumway and Stoffer (1982);
the paper provides the recursive form in Equations A.20-A.21 (Appendix A) which
is algebraically equivalent. The non-recursive form Sigma_cross[tau] =
Sigma_smooth[tau] @ L[tau-1]^T avoids a separate initialization step (Eq A.21)
and is simpler to implement. This equivalence can be verified by substituting
A.20 into the recursive formula -- Researcher inference on the choice of
non-recursive form.

**Implementation detail:** Store L[tau] for tau = 1..N-1 during the backward pass
(array of N-1 2x2 matrices) to avoid recomputation in the cross-covariance step.
...
# --- Seasonality (Paper, Eq A.39 / Eq 24) ---
C = [1, 1]
for i = 1 to I:
    phi[i] = 0.0
    count = 0
    for t = 1 to T:
        tau = (t - 1) * I + i
        if observed[tau]:
            phi[i] += y[tau] - C @ x_smooth[tau]
            count += 1
    phi[i] /= count                              # Eq A.39 uses T; use count for missing data

# --- Observation noise (Paper, Eq A.38) ---
...
for tau = 1 to N:
    if observed[tau]:
        i = phi_position[tau]
        residual = y[tau] - phi[i] - C @ x_smooth[tau]
        r += residual^2 + C @ Sigma_smooth[tau] @ C^T
        N_obs += 1
r /= N_obs                                       # Eq A.38 uses N; use N_obs for missing data
...
The compact form used here -- r = (1/N) * sum[(y - phi - C @ x_smooth)^2 +
...
```
INPUT: y[1..N], observed[1..N], phi_position[1..N], I, T, max_iter, tol
OUTPUT: theta_final
...
for j = 1 to max_iter:
    # E-step
    x_filt, Sigma_filt, x_pred, Sigma_pred, A_used, e, S =
        kalman_filter(y, observed, phi_position, theta)            # Step 2
    x_smooth, Sigma_smooth, Sigma_cross =
    ...
    theta = m_step(x_smooth, Sigma_smooth, P, P_cross,
                   y, observed, phi_position, I, T)                # Step 5
...
#### Step 8: Robust Kalman Filter Variant

Replace the correction step in Step 2 with outlier-aware correction:

```
# In the Kalman filter loop, when observed[tau] is true:

S[tau] = C @ Sigma_pred[tau] @ C^T + r               # scalar predictive variance
e[tau] = y[tau] - C @ x_pred[tau] - phi[i]           # innovation

# Soft-thresholding for sparse outlier detection
threshold = lambda * S[tau] / 2                       # adaptive threshold
...
# Joseph form covariance update (unchanged from standard)
I_2 = [[1, 0], [0, 1]]
temp = I_2 - K @ C
Sigma_filt[tau] = temp @ Sigma_pred[tau] @ temp^T + (K * r) @ K^T
```

**Source:** Paper, Section 3.1, Equations 25-34. The soft-thresholding solution
(Eq 33) is the closed-form solution to the Lasso-penalized minimization problem
(Eq 30). The threshold lambda / (2 * W_{tau+1}) in the paper equals
lambda * S[tau] / 2 because W = 1/S (Eq 30 defines W as the inverse of
the predictive variance scalar). The cleaned innovation e - z* truncates
the distribution, leaving only the portion within the threshold band.

#### Step 9: Robust EM M-step Modifications

Only the phi and r updates change in the robust variant:

```
# phi update (Paper, Eq 36)
for i = 1 to I:
    phi[i] = 0.0
    count = 0
    for t = 1 to T:
...
**Source:** Paper, Section 3.2, Equations 35-36. The modification is minimal:
z_star[tau] is subtracted from the observation residual in both formulas. All
other M-step equations (a_eta, a_mu, sigma_eta_sq, sigma_mu_sq, pi_1, Sigma_1)
are unchanged because z* affects only the observation equation, not the state
dynamics. The smoother (Step 3) operates on the filtered states which already
incorporate the outlier cleaning, so it is also unchanged.

**Note:** Setting lambda = infinity recovers the standard filter exactly (threshold
becomes infinite, z_star is always zero). The robust variant therefore subsumes
the standard one. Paper, Section 3 and Table 1 show that robust performs equal
or better even on clean data (MAPE difference <= 0.01).

#### Step 10: Static Prediction
...
#### Step 11: Dynamic Prediction (One-Bin-Ahead)
...
```
INPUT: volume forecasts updated at each bin, total order quantity Q_total
OUTPUT: shares executed per bin

cumulative_executed = 0.0

for i = 1 to I - 1:
    # Compute current forecasts for remaining bins i..I
    vol_remaining = sum(volume_hat_dynamic[j] for j = i to I)

    # Weight for bin i: proportion of remaining to execute
    w[i] = volume_hat_dynamic[i] / vol_remaining
    ...
    # Observe actual volume for bin i, run Kalman correction (Step 11)
    # Re-forecast bins i+1..I using updated state
...
| A_used          | float64 | (N-1, 2, 2) | Transition matrices used at each step |
| L               | float64 | (N-1, 2, 2) | Smoother gains                        |
| P               | float64 | (N, 2, 2)| Second moment E[x x^T \| all data]      |
| P_cross         | float64 | (N-1, 2, 2) | Cross second moment                  |
...
3. **Robust equals standard on clean data:** With lambda = infinity (or very
   large), the robust filter should produce identical results to the standard
...
| Non-recursive cross-covariance| Researcher inference (Shumway & Stoffer 1982)        |
...
| Phi identifiability           | Researcher inference (implicit in model structure)   |
