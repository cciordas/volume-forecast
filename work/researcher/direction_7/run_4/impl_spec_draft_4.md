...
For tau = 1, 2, ..., N:
    # --- Predict step ---
    # Determine A_tau based on whether this is the initial step or a day boundary.
    # IMPORTANT: tau=1 is NOT a day-boundary transition. The prior pi_1 is the mean
    # of x_1 directly (Paper, page 4, below Eq 5), not a state at "time 0" that must
    # be transitioned forward. Using A=I, Q=0 at tau=1 ensures x_pred[1] = pi_1 and
    # Sigma_pred[1] = Sigma_1, consistent with the paper's initialization and the EM
    # M-step (Eq A.32: pi_1 = x_smooth[1]).
    if tau == 1:                      # initial step: no transition
        A = [[1, 0], [0, 1]]         # identity
        Q = [[0, 0], [0, 0]]         # no process noise
    elif tau mod I == 1:              # day boundary (tau = I+1, 2I+1, ...)
        A = [[a_eta, 0], [0, a_mu]]
...
- The tau=1 special case uses A=I (identity) and Q=0 because pi_1 is the prior mean
  of x_1 directly, not a state at "time 0" requiring a transition. This ensures
  x_pred[1] = pi_1 and Sigma_pred[1] = Sigma_1, which is consistent with the EM
  M-step update pi_1 = x_smooth[1] (Eq A.32). Day-boundary transitions apply only at
  tau = I+1, 2I+1, 3I+1, ..., which transition from the last bin of day d to the first
  bin of day d+1. (Paper, page 4, below Eq 5; Algorithm 1; Appendix A, Eq A.32)
...
- **M-step ordering constraints:** Three ordering dependencies exist in the M-step,
  all established by the (j+1) superscripts in the paper's closed-form solutions:
  1. **phi before r:** Equation A.38 uses phi^{(j+1)}, so the r update requires the
     newly estimated phi from the current M-step, not the old phi. Since phi depends
     only on E-step quantities (Eq A.39), it can be computed first.
  2. **a^eta before (sigma^eta)^2:** Equation A.36 uses (a^eta)^{(j+1)}, so the
     variance update requires the newly estimated AR coefficient.
  3. **a^mu before (sigma^mu)^2:** Equation A.37 uses (a^mu)^{(j+1)}, same dependency.
  The remaining parameters (pi_1, Sigma_1) depend only on E-step sufficient statistics
  and have no M-step ordering dependencies. The pseudocode above respects all three
  constraints: phi is computed before r, a^eta before (sigma^eta)^2, and a^mu before
  (sigma^mu)^2. A developer reordering the M-step updates (e.g., grouping all variance
  updates together) must preserve these three dependencies.
  (Paper, Appendix A.3, Equations A.34-A.39)
...
For each tau in dynamic mode:
    # Predict step: identical to Algorithm 1 (including tau=1 identity transition)
    A, Q = construct_AQ(tau, theta)   # same day-boundary logic (with tau=1 special case)
    A_store[tau] = A
...
**Robust EM convergence criterion:**
The convergence check uses the same innovation-form log-likelihood formula as the
standard EM (Appendix A.1, Eq A.8). The innovations e_tau = y[tau] - y_hat[tau] are
identical to the standard filter (the predict step is unchanged; only the correction
step differs via the cleaned innovation), so the formula is well-defined and computable.

However, the robust EM is not a standard EM algorithm: it alternates between solving
a Lasso subproblem for z_star (coordinate descent on a penalized objective) and updating
theta in closed form. The monotonic log-likelihood increase property of standard EM may
not strictly hold for the robust variant due to the Lasso penalty. As a safeguard,
monitor both:
1. Relative change in innovation-form log-likelihood (same as standard EM).
2. Maximum absolute change in key parameters (a^eta, a^mu, r) between iterations.
Treat convergence as achieved when both the log-likelihood change and the maximum
parameter change are below their respective tolerances.

(Paper, Section 3.2 does not discuss convergence criteria for the robust EM.
Researcher inference: the dual-criterion safeguard is based on standard optimization
theory for penalized EM-like algorithms, where monitoring the objective alone may miss
cycling behavior.)
...
Multi-step prediction loop for dynamic VWAP denominator at bin i:

```
# After correcting with bin i-1's observation (or using the initial state for i=1),
# the current filtered state is x_hat[tau_{i-1}], where tau_{i-1} is the global index
# of bin i-1 on day t.

# All remaining bins are within the same day, so:
A_within_day = [[1, 0], [0, a_mu]]

# Forward propagation for bins i, i+1, ..., I:
x_fwd = x_hat[tau_{i-1}]                       # start from latest filtered state
volume_hat_remaining = zeros(I - i + 1)

for j = i, i+1, ..., I:
    x_fwd = A_within_day @ x_fwd               # predict forward (no correction)
    y_hat_j = C @ x_fwd + phi[j]               # log-volume forecast
    volume_hat_remaining[j - i] = exp(y_hat_j)  # volume forecast

# The denominator for w_{t,i}^(d) is sum(volume_hat_remaining).
# Note: for bin i itself, x_fwd after one propagation from x_hat[tau_{i-1}] gives the
# same one-step-ahead prediction that the Kalman filter would compute at bin i.
```

Note: within a single day, A = [[1,0],[0,a^mu]] and Q plays no role in the point forecast
(only in uncertainty quantification). The loop involves a nested computation: for each
bin i during the day, propagate forward through bins i..I to compute the denominator.
...
4. **Rolling-window estimation:**
   - For each test day d:
     ...
     - Run Kalman filter on day d to produce forecasts (see evaluation modes below).
   ...
   **Out-of-sample evaluation modes:** For each test day d, the filter must first be
   run dynamically through the training window to establish the current state, then
   evaluation proceeds according to the mode:

   - **Static evaluation of day d:** Run the Kalman filter in dynamic mode (with
     corrections) through all bins of days 1 through d-1. At the end of day d-1,
     the filter's posterior state x_hat and Sigma represent the best estimate of the
     current state. Then switch to static mode (no corrections) for bins 1..I of day
     d: the filter produces multi-step-ahead forecasts using only the state at the
     close of day d-1. These are the "static predictions."

   - **Dynamic evaluation of day d:** Run the Kalman filter in dynamic mode through
     all bins including day d. The one-step-ahead predictions y_hat for bins 1..I of
     day d (computed before the correction step at each bin) are the "dynamic
     predictions."

   In both modes, the initial state for day d's evaluation comes from the filter's
   posterior at the end of day d-1. The distinction is whether corrections are applied
   during day d itself.

   (Paper, Section 4.2, first paragraph: "The static prediction refers to forecasting
   all the I bins of day t + 1 using information up to day t only. The dynamic prediction
   stands for the one-bin-ahead forecasting.")
...
VWAP tracking error (VWAP^TE) is defined as (Paper, Section 4.3, Equation 42):
...
replicated_VWAP_t = sum_{i=1}^{I} w_{t,i} * price_{t,i} is the model-based VWAP replica
using predicted weights w_{t,i} (from Eq 40 or 41) and last-transaction prices price_{t,i}
for each bin i of day t. The true VWAP is VWAP_t = sum_{i=1}^{I} volume_{t,i} * price_{t,i}
/ sum_{i=1}^{I} volume_{t,i} (Paper, Section 4.3, Equation 39). Errors are expressed in
basis points (multiply the ratio by 10,000, where 1 bps = 0.01%).

Dynamic VWAP tracking error (weights revised at each bin using Eq 41):
- Robust Kalman Filter: average 6.38 bps.
- Standard Kalman Filter: average 6.39 bps.
- CMEM: average 7.01 bps.

Static VWAP tracking error (weights fixed before market open using Eq 40):
- Robust Kalman Filter: average 6.85 bps.
- Standard Kalman Filter: average 6.89 bps.
- CMEM: average 7.71 bps.

Rolling Mean baseline: average 7.48 bps. Note that RM produces static weights (the
rolling average of historical volume proportions), so it should be compared against the
static VWAP results above, not the dynamic ones. RM has no dynamic counterpart because
it does not update weights intraday.

(Paper, Section 4.3, Table 4, bottom row "Average". Equations 39, 40, 41, 42.)
...
- 15% dynamic VWAP tracking error improvement over Rolling Mean:
  (7.48 - 6.38) / 7.48 = 14.7% ~ 15%. Note: this compares the dynamic KF result
  against the RM static result, which is what the paper reports.
- 9% dynamic VWAP tracking error improvement over CMEM:
  (7.01 - 6.38) / 7.01 = 9.0%.
...
| VWAP tracking error definition | Chen et al. (2016), Section 4.3, Equations 39 and 42 |
| Static VWAP tracking error results | Chen et al. (2016), Table 4, bottom row, static columns |
| RM VWAP tracking error (7.48 bps) | Chen et al. (2016), Table 4, bottom row, RM mean column |
| Evaluation mode transition (static/dynamic) | Chen et al. (2016), Section 4.2, first paragraph |
| M-step ordering: a^eta before sigma^eta | Chen et al. (2016), Appendix A.3, Eq A.36 ((j+1) superscript on a^eta) |
| M-step ordering: a^mu before sigma^mu | Chen et al. (2016), Appendix A.3, Eq A.37 ((j+1) superscript on a^mu) |
...
| Kalman filter tau=1 initialization (no transition) | Chen et al. (2016), page 4, below Eq 5 (pi_1 is prior mean of x_1 directly); Algorithm 1; Appendix A, Eq A.32 |
| Robust EM convergence criterion | Researcher inference (paper Section 3.2 does not discuss convergence; dual-criterion safeguard from optimization theory) |
