...
FUNCTION kalman_filter(y[1..N], theta):
    ...
    x_pred = array of N 2x1 vectors     # x_{tau|tau-1} (predicted)
    Sigma_pred = array of N 2x2 matrices # Sigma_{tau|tau-1} (predicted)
    K_gains = array of N 2x1 vectors     # Kalman gains (needed by smoother)
    innovations = array of N scalars      # for log-likelihood computation
    S_values = array of N scalars         # innovation variances

    # Initialize: set x_{1|0} = pi_1, Sigma_{1|0} = Sigma_1
    x_pred[1] = pi_1       # 2x1 vector
    Sigma_pred[1] = Sigma_1 # 2x2 matrix

    # Log-likelihood accumulator (innovations form)
    log_lik = 0.0
    ...
    FOR tau = 1 TO N:
        ...
        # Accumulate innovations log-likelihood:
        # LL = -0.5 * sum [ log(S_tau) + innovation^2 / S_tau + log(2*pi) ]
        # (Standard Kalman filter log-likelihood identity; derivable from
        #  Paper Eq A.3 by marginalizing over hidden states)
        log_lik = log_lik - 0.5 * (log(S_tau) + innovation^2 / S_tau + log(2*pi))

        # Store for diagnostics and EM convergence monitoring
        innovations[tau] = innovation
        S_values[tau] = S_tau
...
covariances Sigma_{tau|N} given all N observations. It also computes the
cross-covariances Sigma_{tau,tau-1|N} needed by the EM M-step.

**Note on structure:** The paper's Algorithm 3 (lines 4-8) combines the smoother
and sufficient statistics computation into a single backward pass. This spec
separates them for clarity. Both produce identical results.
(Paper, Section 2.3.1, Algorithm 2; Section 2.3.2, Algorithm 3, lines 4-8)

```
FUNCTION kalman_smoother(x_filt, Sigma_filt, x_pred, Sigma_pred, K_gains,
                          theta, N, I):
    ...
    L_gains = array of N-1 2x2 matrices     # smoother gains L_tau, tau = 1..N-1
    Sigma_cross = array of N-1 2x2 matrices # Sigma_cross[k] = Sigma_{k+1,k|N}
                                             # for k = 1, ..., N-1
    ...
    FOR tau = N-1 DOWNTO 1:
        ...
        # Store smoother gain for cross-covariance recursion
        L_gains[tau] = L_tau
    ...
    # === Cross-covariance backward recursion ===
    # Needed for EM M-step sufficient statistic P_{tau,tau-1}.
    # Index convention: Sigma_cross[k] = Sigma_{k+1,k|N} for k = 1, ..., N-1.
    # That is, Sigma_cross[k] is the cross-covariance between time steps k+1
    # and k, conditioned on all N observations.
    #
    # Paper, Appendix A.2, Equations A.20-A.22

    # --- Initialize at k = N-1 (i.e., Sigma_{N,N-1|N}) ---
    # Paper, Equation A.21:
    # Build A_{N-1} (transition matrix for step N-1 -> N)
    IF (N-1) in D:
        A_Nm1 = [[a_eta, 0], [0, a_mu]]
    ELSE:
        A_Nm1 = [[1, 0], [0, a_mu]]

    Sigma_cross[N-1] = (I_2 - K_gains[N] * C) * A_Nm1 * Sigma_filt[N-1]

    # --- Backward recursion for k = N-2 downto 1 ---
    # Paper, Equation A.20:
    # Sigma_{tau,tau-1|N} = Sigma_{tau|tau} * L_{tau-1}^T
    #     + L_tau * (Sigma_{tau+1,tau|N} - A_tau * Sigma_{tau|tau}) * L_{tau-1}^T
    #
    # In our index convention (Sigma_cross[k] = Sigma_{k+1,k|N}):
    # For k = N-2 downto 1, computing Sigma_cross[k] = Sigma_{k+1,k|N}:
    #   Let tau = k+1 in the paper formula. Then:
    #   Sigma_{k+1,k|N} = Sigma_{k+1|k+1} * L_k^T
    #       + L_{k+1} * (Sigma_{k+2,k+1|N} - A_{k+1} * Sigma_{k+1|k+1}) * L_k^T
    #
    #   = Sigma_filt[k+1] * L_gains[k]^T
    #       + L_gains[k+1] * (Sigma_cross[k+1] - A_{k+1} * Sigma_filt[k+1])
    #         * L_gains[k]^T

    FOR k = N-2 DOWNTO 1:
        # Build A_{k+1} (transition matrix for step k+1 -> k+2)
        IF (k+1) in D:
            A_kp1 = [[a_eta, 0], [0, a_mu]]
        ELSE:
            A_kp1 = [[1, 0], [0, a_mu]]

        Sigma_cross[k] = Sigma_filt[k+1] * L_gains[k]^T
            + L_gains[k+1] * (Sigma_cross[k+1] - A_kp1 * Sigma_filt[k+1])
              * L_gains[k]^T
...
FUNCTION em_algorithm(y[1..N], theta_init, I, max_iter, tol):
    ...
    prev_log_lik = -infinity

    FOR j = 1 TO max_iter:
        # === E-step ===
        # Run forward Kalman filter (Algorithm 1):
        x_filt, Sigma_filt, x_pred, Sigma_pred, K_gains, log_lik,
            innovations, S_values = kalman_filter(y, theta)

        # Run backward smoother (Algorithm 2):
        x_smooth, Sigma_smooth, Sigma_cross, L_gains =
            kalman_smoother(x_filt, Sigma_filt, x_pred, Sigma_pred,
                            K_gains, theta, N, I)
        ...
        #
        # Index convention for Sigma_cross: Sigma_cross[k] = Sigma_{k+1,k|N}.
        # So P_cross[tau] uses Sigma_cross[tau-1] = Sigma_{tau,tau-1|N}. Correct.
        ...
        # NOTE: phi must be computed BEFORE r, since r depends on phi_new.
        FOR i = 1 TO I:
        ...
        # Check convergence using innovations log-likelihood:
        # log_lik was computed in the forward filter pass as:
        #   log_lik = -0.5 * sum_{tau=1}^{N} [ log(S_tau) + innovation_tau^2 / S_tau
        #                                       + log(2*pi) ]
        # This is the marginal log-likelihood of the observations under the
        # current parameters (standard Kalman filter identity; see Shumway and
        # Stoffer (1982) or derivation from Paper Eq A.3).
        # EM guarantees this quantity is non-decreasing across iterations.

        IF j > 1:
            relative_change = abs(log_lik - prev_log_lik) / (abs(prev_log_lik) + 1e-10)
...
# Compute phi BEFORE r, since r depends on phi_new.
FOR i = 1 TO I:
    phi_new[i] = (1 / T) * sum_{t=1}^{T} of
        ( y_{t,i} - C * x_smooth[tau(t,i)] - z_star[tau(t,i)] )

# Modified observation noise variance (Paper, Equation 35):
# Derived from the robust observation equation y = Cx + phi + v + z.
# The residual v = y - Cx - phi - z, so E[v^2] expands as:
#   E[(y - Cx - phi - z)^2]
#   = y^2 + E[(Cx)^2] + phi^2 + z^2
#     - 2y*E[Cx] - 2y*phi - 2y*z
#     + 2*phi*E[Cx] + 2*z*E[Cx] + 2*z*phi
#
# Using E[(Cx)^2] = C*P_tau*C^T and E[Cx] = C*x_hat_tau:

r_new = (1 / N) * sum_{tau=1}^{N} of
    { y[tau]^2 + C * P[tau] * C^T
      + (phi_new[bin(tau)])^2 + (z_star[tau])^2
      - 2 * y[tau] * C * x_smooth[tau]
      - 2 * y[tau] * phi_new[bin(tau)]
      - 2 * y[tau] * z_star[tau]
      + 2 * phi_new[bin(tau)] * C * x_smooth[tau]
      + 2 * z_star[tau] * C * x_smooth[tau]
      + 2 * z_star[tau] * phi_new[bin(tau)] }
```
...
**Convergence monitoring for robust EM:** The standard EM (Algorithm 3) monitors
convergence via the innovations log-likelihood computed in Algorithm 1. For the
robust EM, Algorithm 4 produces cleaned innovations (e_clean = e_tau - z_star).
Use the cleaned-innovation log-likelihood for convergence monitoring:

    LL_robust = -0.5 * sum_{tau=1}^{N} [ log(S_tau) + e_clean[tau]^2 / S_tau + log(2*pi) ]

where S_tau and e_clean[tau] are stored during the robust forward pass. This
measures how well the state-space model fits the data after outlier removal.
Alternatively, monitor parameter convergence directly:
max(|theta_new - theta_old| / (|theta_old| + epsilon)) < tol, where epsilon is a
small constant (e.g., 1e-10) to avoid division by zero for parameters near zero.
(Researcher inference: the paper does not specify the convergence criterion for
the robust EM. The cleaned-innovation LL is the most natural analog of the
standard EM's innovations LL. The parameter-change criterion is a robust fallback
that does not depend on the LL form.)
...
FUNCTION dynamic_vwap_weights(x_filt_prev, theta, current_bin, I,
                               weights_so_far):
    # Paper, Equation 41
    # At bin i (1-indexed), we have observed bins 1..i-1 and need forecasts
    # for bins i..I.
    #
    # INPUT: x_filt_prev is the filtered state AFTER observing bin i-1,
    #        i.e., x_{(i-1)|(i-1)}. This is the state produced by the Kalman
    #        filter's correction step at the previous bin. At the start of
    #        the day (i=1), this is the filtered state from the last bin of
    #        the previous day, x_{N_prev|N_prev}.
    #
    # The dynamic VWAP strategy requires multi-step-ahead forecasts from the
    # current bin i to the last bin I of the day. These are obtained by running
    # the prediction-only forward pass from x_filt_prev (no correction steps),
    # similar to static_forecast but starting mid-day.
    # (Researcher inference based on Paper Eq 41, which uses volume_{t,j}^{(d)}
    #  for j = i to I; one-step-ahead forecasts are only available for bin i,
    #  so bins i+1 through I require multi-step prediction.)

    i = current_bin

    IF i == I:
        # Last bin: execute remaining fraction
        weight = 1.0 - sum(weights_so_far)
        RETURN weight

    # Produce multi-step forecasts from x_filt_prev for bins i..I
    x_curr = x_filt_prev
    remaining_forecasts = array of (I - i + 1) values

    FOR h = 0 TO (I - i):
        target_bin = i + h
        IF h == 0 AND i == 1:
            # Day-boundary transition: x_filt_prev is the filtered state from
            # the last bin of the previous day, so the first prediction step
            # crosses a day boundary. Use day-boundary dynamics for eta.
            # (Paper, Section 2, Equations 4-5; consistent with
            # static_forecast handling of h==1 and Algorithm 1 day-boundary
            # logic.)
            A = [[a_eta, 0], [0, a_mu]]
        ELSE:
            # Within-day transition: eta held constant, mu evolves via AR(1)
            A = [[1, 0], [0, a_mu]]
        x_next = A * x_curr
    ...
    vol_remaining = array of (I - i + 1) values
    FOR h = 0 TO (I - i):
        vol_remaining[h] = exp(remaining_forecasts[h])

    # Weight for bin i:
    # w_{t,i} = vol_remaining[0] / sum(vol_remaining) * (1 - sum(weights_so_far))
    pred_i = vol_remaining[0]
    total_remaining = sum(vol_remaining)
    already_executed = sum(weights_so_far)
    weight = (pred_i / total_remaining) * (1.0 - already_executed)
...
Input: raw_volume[t, i] for t = 1..T, i = 1..I
       ...
       (shares_outstanding is the most recently reported value as of day t,
        typically from a corporate actions database or previous day's closing
        record. Most days it is unchanged; it updates only on corporate actions
        such as stock splits, buybacks, or new issuance.)
...
Step 5: Convert to linear scale
    ...
    Note on Jensen's inequality bias: For a log-normal random variable,
    E[X] = exp(E[log X] + Var[log X] / 2), so exp(E[log X]) systematically
    underestimates E[X]. The paper uses exp(y_hat) without correction and
    achieves good MAPE results (Table 3). The bias-corrected version would be
    exp(y_hat + S_tau / 2) where S_tau is the innovation variance, but this was
    not used in the paper's evaluation and is not part of the specified model.
    (Researcher inference; standard log-normal property.)
...
Step 7: Evaluate forecast accuracy
    MAPE (Paper, Eq 37):
        MAPE = (1/M) * sum_{tau=1}^{M} |volume_tau - predicted_volume_tau|
                                         / volume_tau
    where M is the number of out-of-sample bins, volume_tau is the actual
    linear-scale volume, and predicted_volume_tau is the forecast in linear
    scale (i.e., after exponentiation in Step 5). MAPE is computed on
    linear-scale volumes, NOT log-scale.

    VWAP tracking error (Paper, Eq 42):
        VWAP_t = sum_{i=1}^{I} (price_{t,i} * volume_{t,i})
                 / sum_{i=1}^{I} volume_{t,i}
        replicated_VWAP_t = sum_{i=1}^{I} w_{t,i} * price_{t,i}
        VWAP^TE = (1/D) * sum_{t=1}^{D} |VWAP_t - replicated_VWAP_t| / VWAP_t
    where D is the number of out-of-sample days, price_{t,i} is the last
    recorded transaction price in bin i of day t (Paper, Section 4.1,
    Equation 1), and w_{t,i} are the execution weights from Algorithm 6.
    (Paper, Section 4.3, Equations 39, 42)
...
standard Kalman filter as the special case lambda -> infinity: as lambda grows,
the threshold lambda/(2*W_tau) grows without bound, so z_star = 0 always
(no outliers ever detected), and the robust correction step reduces exactly to
the standard Kalman correction. Conversely, lambda -> 0 yields threshold -> 0,
so z_star -> e_tau always (all innovations are absorbed as outlier noise, and
the Kalman correction is effectively disabled -- the filter ignores observations).
(Paper, Section 3.1, Equations 33-34)
...
- For comparison: CMEM achieves 0.65 dynamic / 0.90 static; rolling mean
  achieves 1.28 static (RM is inherently static; Paper, Table 3 reports it
  only in the static section).
...
- Static VWAP: average 6.85 basis points for robust Kalman filter
  (Paper, Section 4.3, Table 4).
- For comparison: CMEM achieves 7.01 dynamic / 7.71 static; rolling mean (RM)
  achieves 7.48 static (RM is inherently static -- it produces a fixed volume
  profile from historical averages; no dynamic variant exists in Table 4).
  (Paper, Table 4, "Average" row.)
- The paper's reported 15% improvement is dynamic RKF (6.38) vs static RM (7.48),
  a cross-strategy comparison. The 9% improvement is dynamic RKF (6.38) vs
  dynamic CMEM (7.01). (Paper, Section 4.3, final paragraph.)
  Verification: 6.38 / 0.85 = 7.51 (close to RM = 7.48); 6.38 / 0.91 = 7.01
  (matches CMEM dynamic).
...
3. **EM convergence:** Log-likelihood (innovations form, computed in the forward
   filter) should increase monotonically at each EM iteration. If it decreases,
   there is a bug in the E-step or M-step.
...
| Innovations log-likelihood | Derivable from Paper Eq A.3; standard identity per Shumway and Stoffer (1982) |
...
| Cross-covariance initialization (Eq A.21) | Chen et al. (2016), Appendix A.2, Equation A.21 |
| Cross-covariance recursion (Eq A.20) | Chen et al. (2016), Appendix A.2, Equation A.20 |
| Cross-covariance sufficient statistic (Eq A.22) | Chen et al. (2016), Appendix A.2, Equation A.22 |
...
| Jensen's inequality bias | Researcher inference (standard log-normal property) |

## Revision History

### Draft 2 (revision from critique 1)

**Issues addressed:**

- **M1 (cross-covariance pseudocode):** Completely rewrote Algorithm 2 cross-covariance
  section. Now contains a single, self-contained backward loop with: (a) explicit
  storage of all smoother gains L_tau in the smoothing pass, (b) initialization at
  k=N-1 using Eq A.21, (c) backward recursion from k=N-2 downto 1 using Eq A.20
  with explicit index mapping (Sigma_cross[k] = Sigma_{k+1,k|N}). Removed the
  prose "detailed formula" section; everything is now inside the pseudocode block.
  (Addresses m1 and m6 as well.)

- **M2 (robust r update sign error):** Removed the incorrect first version of the
  r update formula (which had -2*(z_star)^2 instead of +(z_star)^2). Kept only the
  correct formula derived from expanding E[(y - Cx - phi - z)^2], with explicit
  derivation showing why (z_star)^2 is positive. (Paper, Eq 35 / Eq A.38 extended.)

- **M3 (lambda limiting behavior):** Corrected the Variants section. Removed the
  false claim that "lambda = 0 with z_star always zero." Now correctly states:
  lambda -> infinity means threshold -> infinity, so z_star = 0 always (standard KF);
  lambda -> 0 means threshold -> 0, so z_star -> e_tau (all innovations absorbed as
  outlier noise, Kalman correction effectively disabled). (Paper, Eqs 33-34.)

- **M4 (VWAP benchmarks):** Corrected CMEM dynamic VWAP tracking error from 8.97
  to 7.01 bps. Added verification cross-check against Section 4.3 text (15%
  improvement over RM, 9% over CMEM). (Paper, Section 4.3, Table 4 Average row;
  Section 4.3 final paragraph.) Note: RM static attribution corrected in draft 3.

- **M5 (missing log-likelihood formula):** Added innovations log-likelihood computation
  inside the forward filter loop (Algorithm 1). Formula: LL = -0.5 * sum[log(S_tau) +
  innovation^2/S_tau + log(2*pi)]. Added convergence check in Algorithm 3 using
  relative change in this quantity. Cited derivation from Paper Eq A.3 and Shumway
  and Stoffer (1982).

- **m1 (L_gains not stored):** Added explicit L_gains array declaration and storage
  inside the smoother backward loop.

- **m2 (MAPE formula):** Added MAPE formula to Data Flow Step 7, specifying it operates
  on linear-scale volumes after exponentiation.

- **m3 (VWAP tracking error formula):** Added full VWAP^TE formula to Data Flow Step 7,
  including the VWAP definition (Eq 39) and replicated VWAP.

- **m4 (Jensen's inequality bias):** Added note in Data Flow Step 5 acknowledging the
  bias and stating the paper does not apply correction.

- **m5 (dynamic VWAP multi-step forecasts):** Rewrote dynamic_vwap_weights function to
  explicitly produce multi-step-ahead forecasts from the current filtered state for
  bins i through I, rather than assuming pre-computed forecasts as input.

- **m6 (cross-covariance index convention):** Added explicit index mapping comment:
  Sigma_cross[k] = Sigma_{k+1,k|N} for k = 1, ..., N-1. Documented throughout
  Algorithm 2 and Algorithm 3's sufficient statistics section.

- **m7 (shares outstanding):** Added clarification in Data Flow input section that
  shares_outstanding is the most recently reported value, typically constant except
  on corporate action dates.

- **m8 (smoother/EM structural difference):** Added note at top of Algorithm 2 stating
  the paper combines smoother and sufficient statistics in one pass but the spec
  separates them for clarity.

### Draft 3 (revision from critique 2)

**Issues addressed (4 minor, 0 major):**

- **m1 (VWAP benchmark misattributions):** Corrected two errors in the VWAP tracking
  error comparison table: (a) Added CMEM static = 7.71 bps (was listed as "not
  separately reported" but IS in Table 4 Average row). (b) Changed RM from "dynamic"
  to "static" -- rolling mean is inherently a static strategy (fixed profile from
  historical averages; no dynamic variant in Table 4). Clarified that the paper's
  15% improvement compares dynamic RKF (6.38) vs static RM (7.48), a cross-strategy
  comparison. (Paper, Section 4.3, Table 4, Average row.)

- **m2 (Robust EM convergence undefined):** Added explicit convergence monitoring
  specification for Algorithm 5 (robust EM). Primary criterion: cleaned-innovation
  log-likelihood LL_robust = -0.5 * sum[log(S_tau) + e_clean^2/S_tau + log(2*pi)],
  where e_clean = e_tau - z_star from the robust forward pass. Fallback criterion:
  max relative parameter change < tol. Marked as Researcher inference since the
  paper does not specify robust EM convergence.

- **m3 (dynamic VWAP input semantics):** Renamed x_filt_current to x_filt_prev and
  added explicit documentation that this is the filtered state after observing bin
  i-1 (i.e., x_{(i-1)|(i-1)}), not the state at the current bin. Removed
  Sigma_filt_current parameter (unused in forecast computation). Added note about
  what x_filt_prev is at the start of the day (i=1).

- **m4 (redundant IF/ELSE in dynamic_vwap_weights):** Collapsed identical h==0 and
  h>0 branches into a single loop body. All iterations use the same within-day
  transition (eta constant, mu AR(1)), making the code unambiguous.

### Draft 4 (revision from critique 3)

**Issues addressed (3 minor, 0 major):**

- **m1 (day-boundary transition in dynamic_vwap_weights for i=1):** Added a
  conditional for the first prediction step when current_bin == 1. When i=1,
  x_filt_prev is the filtered state from the last bin of the previous day, so the
  first transition crosses a day boundary and must use A = [[a_eta, 0], [0, a_mu]]
  instead of the within-day matrix. This is consistent with static_forecast (which
  correctly uses the day-boundary transition at h==1) and Algorithm 1 (which applies
  day-boundary dynamics when tau is in D). The practical impact is small (a_eta is
  typically close to 1.0, and at bin 1 the dynamic strategy has no information
  advantage over static), but this fixes a correctness defect in the forecast at the
  first bin of each day. (Paper, Section 2, Equations 4-5.)

- **m2 (RM MAPE benchmark phrased imprecisely):** Changed "rolling mean achieves 1.28
  for both" to "rolling mean achieves 1.28 static (RM is inherently static; Paper,
  Table 3 reports it only in the static section)." This makes the MAPE section
  consistent with the VWAP section's already-correct treatment of RM as an
  inherently static method. (Paper, Table 3, Average row -- RM appears only in the
  Static Volume Prediction section.)

- **m3 (Algorithm 1 RETURN omits innovations and S_values):** Added innovations and
  S_values to Algorithm 1's return signature. These arrays are already computed and
  stored during the forward pass (lines 159-160) and are needed for: (a) sanity
  check #4 (innovation ACF diagnostics), and (b) optional Jensen's inequality bias
  correction (exp(y_hat + S_tau/2)). Also updated the E-step call in Algorithm 3 to
  accept the expanded return tuple.
