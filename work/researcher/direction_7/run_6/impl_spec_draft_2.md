...
FUNCTION dynamic_vwap_weights(x_filt_current, Sigma_filt_current, theta,
                               current_bin, I, weights_so_far):
    # Paper, Equation 41
    ...
    # the prediction-only forward pass from the current filtered state (no
    # correction steps), similar to static_forecast but starting from the
    # filtered state at bin i-1 rather than the end of the previous day.
    ...
    # Produce multi-step forecasts from current state for bins i..I
    # Start from filtered state at bin i-1 (or predicted state at bin i)
    ...
    FOR h = 0 TO (I - i):
        target_bin = i + h
        # Determine transition (within-day since we are mid-day)
        IF h == 0:
            # First step: prediction from bin i-1 to bin i (already done)
            # Use x_pred for bin i as the forecast
            # Actually, at this point we have x_filt at bin i-1
            A = [[1, 0], [0, a_mu]]
            Q = [[0, 0], [0, sigma_mu_sq]]
            x_next = A * x_curr
...
- For comparison: CMEM achieves 7.01 dynamic / not separately reported static;
  rolling mean (RM) achieves 7.48 dynamic (Paper, Table 4, "Average" row).
- The paper text confirms: "an improvement of 15% compared with the RM, and
  of 9% when compared with the dynamic CMEM" (Section 4.3, final paragraph).
...
- **M4 (VWAP benchmarks):** Corrected CMEM dynamic VWAP tracking error from 8.97
  to 7.01 bps. Corrected RM dynamic from implicit wrong value to 7.48 bps. Added
  verification cross-check against Section 4.3 text (15% improvement over RM, 9%
  over CMEM). (Paper, Section 4.3, Table 4 Average row; Section 4.3 final paragraph.)
