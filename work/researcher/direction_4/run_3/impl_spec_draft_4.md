...
function ForecastRawVolume(symbol s, day t, bin i, current_bin):
    ...
    if intraday_model is not None and current_bin >= 1:
        ...
        # PURITY REQUIREMENT: model.forecast(steps=N) MUST be a pure
        # function that does NOT modify the model's persistent ar_buffer
        # or ma_buffer. It should operate on internal copies of the
        # buffers to compute multi-step forecasts. This ensures that
        # calling ForecastRawVolume multiple times with different target
        # bins but the same current_bin produces independent, correct
        # results. The reset-and-reprocess design in ConditionIntraDayARMA
        # makes this self-healing in practice, but forecast() purity is
        # still required for correctness if conditioning is ever hoisted
        # outside the inner loop (see Function 9 optimization note).
        # [Researcher inference.]
        # Re-seasonalize
...
function TrainPercentageRegression(symbol s, training_days):
    ...
    for d in training_days:
        ...
        #
        # STRUCTURAL ASYMMETRY WITHIN SURPRISES:
        # There is a third, subtler mismatch: whether actual_pct and
        # expected_pct within a single surprise share the SAME denominator.
        # - In prediction (Function 7): both use V_total_est, so
        #   surprise = (actual - forecast) / V_total_est (common denom).
        # - In training (below): actual uses daily_total while expected
        #   uses lag_forecast_total (different denoms). The surprise does
        #   NOT simplify to a ratio over a common denominator.
        # The same asymmetry applies to the target variable (actual uses
        # daily_total, expected uses forecast_total_d).
        # Impact is small: daily_total and forecast totals converge when
        # the raw model is well-calibrated, and regression coefficients
        # are small (|beta| < 0.5). To eliminate entirely, the developer
        # can use the same denominator for both terms within each
        # surprise during training (e.g., use daily_total for both
        # actual and expected). This is a trivial change and would make
        # the surprise formula structurally identical between training
        # and prediction.
        # [Researcher inference.]
...
function DailyOrchestration(symbol s, day t):
    ...
    for current_bin in 1..I:
        ...
        #
        # OPTIMIZATION NOTE: Each call to ForecastRawVolume internally
        # calls ConditionIntraDayARMA with the same deseas_observed
        # array (since current_bin is constant across this inner loop).
        # With the reset-and-reprocess design, this re-conditions the
        # model identically (I - current_bin) times per outer iteration.
        # The pseudocode is correct by construction (each conditioning
        # call resets state before reprocessing), but the developer may
        # hoist the conditioning step outside the inner loop for
        # efficiency:
        #
        #   deseas_observed = [volume[s,t,j]/seasonal_factor[j] for j in 1..current_bin]
        #   ConditionIntraDayARMA(intraday_model, deseas_observed)
        ...
        #
        # If hoisting, the forecast() purity requirement (Function 6
        # note) becomes critical: forecast(steps=N) must not mutate
        # the model's buffers, or the second call in the loop would
        # see corrupted state.
        # Similarly, ForecastVolumePercentage (Function 7) calls
        # ForecastRawVolume internally, which re-conditions the model
        # at different current_bin values. The final model state after
        # ForecastVolumePercentage returns may differ from the state
        # set by the Model A loop. This is harmless because the next
        # outer iteration re-conditions from scratch.
        # [Researcher inference.]
...
| Forecast purity requirement (Function 6) | Researcher inference |
| Redundant conditioning optimization (Function 9) | Researcher inference |
| Surprise denominator structural asymmetry (Function 8) | Researcher inference |
