...
function TrainPercentageRegression(symbol s, training_days):
    ...
    for d in training_days:
        # Compute daily total for this training day (exact, known in hindsight).
        # NOTE: This uses the actual daily total as the denominator for
        # computing percentages, which differs from prediction time where
        # V_total_est is used. See Function 7, Step 3 for discussion of
        # this train/predict mismatch and why it is acceptable.
