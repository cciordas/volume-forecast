...
coupled: Model A's historical average component provides the surprise signal
consumed by Model B.
...
Changes from Draft 1 in response to Critique 1:

- **M1 resolved:** Separated Component 1 historical average (N_hist = 21 days)
  from deseasonalization basis (N_seasonal = 126 days). Added new Function 1a
  (`compute_historical_average`) using N_hist. Function 1
  (`compute_seasonal_factors`) retained for deseasonalization only. Removed
  N_hist from parameter table ambiguity.
- **M2 resolved:** Standardized regime assignment to use last-observed bin and
  cumulative volume through that bin in both training (Function 5) and
  prediction (Function 6). Training now calls `assign_regime(classifier, i-1, cumvol)`.
- **M3 resolved:** Specified independent-segment approach for day-boundary
  handling in intraday ARMA. Each day is a separate segment; ARMA state resets
  at day boundaries. Pooled MLE across segments. Added initial prediction state
  specification for bin 1.
- **M4 resolved:** Changed surprise baseline from unconditional Model A forecast
  to Component 1 historical average (H), matching Humphery-Jenner's original
  "departures from a historical average" formulation. Documented alternative
  approaches.
- **M5 resolved:** Added discussion of percentage coherence and optional
  renormalization step. Noted that VWAP algorithms typically use next-bin
  forecasts only, making full-day coherence non-critical.
- **m1 resolved:** N_hist now used in Function 1a; parameter table consistent.
- **m2 resolved:** Added note about Exhibit 1 "4 Bins" diagram notation.
- **m3 resolved:** Marked surprise std dev range as Researcher inference.
- **m4 resolved:** Clarified that conditioned training MAPE is intentional
  (operational setup, not data leakage).
- **m5 resolved:** Added multi-restart and convergence fallback for Nelder-Mead.
- **m6 resolved:** Specified remainder-day handling for blocked CV.
- **m7 resolved:** Clarified VWAP bps benchmarks are relative, not absolute.
- **m8 resolved:** Defined concrete ARMA predict interface with predict_at,
  predict_next, and append_observation semantics.
...
historical average component and produces a forecast of what fraction of the
day's total volume will trade in the next bin. It adjusts a static historical
percentage curve using recent volume surprises (deviations of actual volume from
the historical average), subject to deviation constraints and a late-day
switch-off rule.
...
21-day average used as Component 1 (H) in the weighted combination and as the
surprise baseline for Model B. The paper treats these as separate quantities:
Exhibit 1 shows "Prior 21 days" for the Historical Window and the text says
"trailing six months" for deseasonalization (p.17 para 5). A shorter window for
Component 1 makes it more responsive to recent volume changes, while a longer
window for deseasonalization provides a more stable reference for removing the
intraday pattern.
...
```
FUNCTION optimize_regime_weights(volume_history, hist_avg, seasonal_factors,
    ...
    FOR d IN trailing N_weight_train days:
        ...
        FOR i = 1 TO I:
            ...
            # Component 1: Historical rolling average (H) — from N_hist window
            H = hist_avg[i]
...
surprise on lagged surprises. Surprises are computed relative to the historical
average (Component 1, H), following Humphery-Jenner's (2011) "departures from
a historical average" formulation.

```
FUNCTION train_percentage_model(model_a, volume_history, N_regression_fit,
                                 L_max, min_volume_floor)
        -> percentage_model
    # Input:  trained Model A (for hist_avg), volume history, regression window, max lags
    ...
    # Surprise baseline = Component 1 (H, historical average from N_hist window)
    # This follows Humphery-Jenner (2011): "departures from a historical average"
    # and Satish et al. p.18-19: "departures from a historical average approach"
    surprise_data = []   # list of (day, bin, surprise_value)
    FOR d IN trailing N_regression_fit days:
        FOR i = 1 TO I:
            H = model_a.hist_avg[i]
            actual = volume_history[d, i]
            IF H <= min_volume_floor:
                surprise = 0.0
            ELSE:
                surprise = (actual - H) / H
...
model terms for U.S. stocks in our in-sample data"). The surprise baseline
choice: Satish et al. p.18-19 says "departures from a historical average
approach" and the Humphery-Jenner (2011) method uses "volume surprises —
deviations from a naive historical forecast." Both phrasings point to Component 1
(H, the historical rolling average) as the surprise baseline, not the full
Model A forecast.

**Surprise baseline alternatives (documented for completeness):**
- (a) **H-only baseline (implemented):** surprise = (actual - H) / H. Matches
  Humphery-Jenner's original formulation. Produces larger surprises (H is less
  accurate than full Model A), which strengthens the regression signal.
- (b) **Full Model A conditioned baseline:** surprise = (actual - V_hat_cond) /
  V_hat_cond. Most informative at prediction time, but requires conditioned
  forecasts during training (computationally expensive and requires observed
  data propagation).
- (c) **Full Model A unconditional baseline (Draft 1 approach):** surprise =
  (actual - V_hat_uncond) / V_hat_uncond. Consistent across training/prediction
  but produces smaller surprises than H-only.

The paper's statement "we could apply our more sophisticated raw volume
forecasting model described previously as the base model from which to compute
volume surprises" (p.19) suggests they tested option (b) or (c) but does not
confirm which was used. Option (a) is the most consistent with the paper's
primary characterization of the method as "departures from a historical average."
...
which is the correct behavior (if the historical average has been accurate
recently, no dynamic adjustment is needed). Researcher inference on applying
this to the surprise regression.
...
VWAP adjustment. Surprises are computed relative to the historical average (H).
...
FUNCTION forecast_volume_percentage(model_a, pct_model, stock, day,
    ...
    # Step 2: Compute surprise vector from historical average baseline
    # Use H (Component 1) as surprise baseline — consistent with training
    surprises = []
    FOR j = 1 TO current_bin:
        H = model_a.hist_avg[j]
        actual = observed_volumes[j]
        IF H <= min_volume_floor:
            surprises.append(0.0)
        ELSE:
            surprises.append((actual - H) / H)
...
**Percentage coherence discussion (M5 resolution):**
...
in Step 1, H-based for surprises in Step 2) is Researcher inference. V_total_est
uses the full conditioned Model A because it needs the best available volume
estimate; surprises use H because that matches the training baseline.
...
FUNCTION train_full_model(volume_history, stock, train_end_date, params)
    ...
    # Step 1a: Historical average (Component 1 — N_hist window)
...
- N_hist = 21: Satish et al. 2014, Exhibit 1 caption ("Prior 21 days"). This is the window for Component 1 (H), the historical average used in the weighted combination and as the surprise baseline. The paper introduces N as "a variable that we shall call N" (p.16) without disclosing the optimized value; 21 is the illustrative value from Exhibit 1. Note: N_hist and N_seasonal serve different purposes and should generally differ — N_hist should be shorter (more responsive) while N_seasonal should be longer (more stable deseasonalization).
...
6. **Percentage model:** Trained last because it depends on Model A's hist_avg
...
9. **Surprise mean near zero:** The mean of surprise values across the training
   set should be approximately zero (balanced over- and under-predictions).
   Researcher inference: std dev should be approximately 0.3-0.6 for liquid
   stocks (since surprise = (actual - H) / H, and H is a 21-day average, daily
   volume typically deviates by 30-60% from its recent mean).

10. **Surprise regression coefficients:** |beta_k| < 0.5 for all k. Positive
    beta_1 expected (positive autocorrelation in surprises: a bin that exceeded
    the historical average suggests the next bin will also exceed it).
...
10. **Lookahead bias in Model B training:** Model A is trained on data through
    train_end_date, then used to compute hist_avg-based surprises for earlier
    days in the surprise regression training window. The bias is negligible
    because the 21-day rolling average changes minimally when dropping one day.
...
| Surprise baseline = historical average | Satish et al. 2014, p.18-19 ("departures from a historical average approach"); Humphery-Jenner 2011 |
...
- Surprise std dev range (~0.3-0.6 for liquid stocks with H baseline)
