...
Changes from Draft 3 in response to Critique 3:

- **M1 resolved:** Fixed volume_history slicing in Function 9 to pass an extended
  slice (including N_hist days before the first training day) to
  optimize_regime_weights, so Function 5's rolling H_d computation has sufficient
  history for all training days. Updated both the grid search call and the final
  re-optimization call. Added documentation to Function 5 signature noting the
  data requirement.
- **m1 resolved:** Changed delta application in Function 8 to scale delta together
  with hist_pct: `adjusted = scale * (hist_pct[next_bin] + delta)`. The deviation
  clamp is applied before scaling, operating on the unscaled percentage-space
  quantities. This ensures the surprise regression output (a percentage-point
  departure from full-day participation rate) is treated consistently with the
  base rate before the remaining-fraction scaling is applied.
- **m2 resolved:** After regime count selection via grid search, weights are now
  re-optimized on the full N_weight_train window (not just the reduced
  training-minus-validation subset). The regime classifier is also rebuilt on
  the full window. This follows standard practice of using the validation split
  only for hyperparameter selection, then re-fitting on all available data.
- **m3 resolved:** Revised the no-intercept validity note to clarify that mean
  surprise is exactly zero at training time by construction (hist_pct is the
  mean of actual_pct over the training window). The warning now applies only to
  production drift between re-estimations. Updated Sanity Check 9 to match.
...
FUNCTION optimize_regime_weights(volume_history, N_hist, seasonal_factors,
    ...
    #         volume_history: must extend at least N_hist days BEFORE the
    #           first training day (Draft 4 M1 fix). The last N_weight_train
    #           days are the training days; earlier days are pre-context for
    #           rolling H_d computation. Total length needed: N_hist + N_weight_train.
    ...
    #
    # NOTE (Draft 4 fix for M1): The caller must pass volume_history that
    # includes at least N_hist days before the first training day. The
    # training loop uses volume_history[:d] to compute H_d, which requires
    # data extending N_hist days before d. For the earliest training day
    # (d = d_0), this means N_hist days before d_0 must be present.
...
**No-intercept validity requirement (Draft 4 m3 revision):** The no-intercept
assumption requires that mean surprise is approximately zero. At training time,
this holds exactly by construction: hist_pct[i] is the mean of actual_pct[d, i]
over the N_regression_fit training days, so mean(surprise[d, i]) = mean(actual_pct
[d, i]) - hist_pct[i] = 0 for each bin i. The no-intercept assumption is therefore
automatically satisfied at training time regardless of window choices.

Between re-estimations, hist_pct is frozen while actual participation rates may
drift, potentially introducing a nonzero mean in prediction-time surprises. The
21-day re-estimation interval (reestimation_interval) limits this drift. If mean
prediction-time surprise deviates significantly from zero (e.g., |mean| > 0.001
computed over the last N days of live predictions), consider triggering early
re-estimation. See Sanity Check 9 for verification.
...
FUNCTION forecast_volume_percentage(model_a, pct_model, stock, day,
    ...
    # NOTE (Draft 4 m1 fix): delta is a predicted percentage-point departure
    # from the UNSCALED hist_pct (since the regression was trained on
    # unscaled participation-rate surprises). We first clamp delta against
    # the unscaled hist_pct, then apply scaling to the combined quantity.
    # This ensures the entire adjusted participation rate (base + departure)
    # is scaled uniformly to the remaining-fraction space, avoiding
    # domain mixing between scaled and unscaled quantities.
    base_pct = pct_model.hist_pct[next_bin]
...
FUNCTION train_full_model(volume_history, stock, train_end_date, params)
    ...
    FOR n_reg IN params.regime_candidates:
        ...
        # NOTE (Draft 4 M1 fix): Pass extended slice that includes N_hist
        # days before the first training day, so rolling H_d has sufficient
        # pre-context for all training days (including the earliest).
        train_days_start = first day of train_days
        extended_start = train_days_start - params.N_hist
        weights = optimize_regime_weights(
            volume_history[extended_start:end_of_train_days], params.N_hist,
            seasonal_factors, interday_models, intraday_model,
            regime_classifier, len(train_days), params.min_volume_floor)
    ...
    # NOTE (Draft 4 m2 fix): Re-optimize weights on the full N_weight_train
    # window after selecting the best regime count. The grid search used a
    # reduced window (N_weight_train minus 21 validation days) for each
    # candidate; now that the hyperparameter (n_reg) is selected, we re-fit
    # on all available training data for better final weights.
    # NOTE (Draft 4 M1 fix): Pass extended slice including N_hist pre-context.
    final_regime_classifier = build_regime_classifier(
        volume_history[:train_end_date], params.N_regime_window, n_reg)
    full_train_start = train_end_date - params.N_weight_train
    full_extended_start = full_train_start - params.N_hist
    weights = optimize_regime_weights(
        volume_history[full_extended_start:train_end_date], params.N_hist,
        seasonal_factors, interday_models, intraday_model,
        final_regime_classifier, params.N_weight_train,
        params.min_volume_floor)
    regime_classifier = final_regime_classifier
...
does not discuss validation methodology. The re-optimization of weights on the
full N_weight_train window after regime count selection (Draft 4 m2 fix) is
Researcher inference — standard practice in model selection: use validation
split only for hyperparameter selection, then re-fit on all available data.
...
9. **Surprise mean near zero:** At training time, the mean of percentage-space
   surprise values across the training set is exactly zero by construction
   (hist_pct is the mean of actual_pct over the training window). This is a
   tautology, not something to verify. However, during live prediction, the mean
   of prediction-time surprises can drift from zero as actual participation rates
   diverge from the frozen hist_pct. Monitor prediction-time mean surprise; if
   |mean| > 0.001 computed over the last N prediction days, consider triggering
   early model re-estimation. Researcher inference: std dev of training surprises
   should be approximately 0.005-0.015 for liquid stocks (individual bin
   participation rates typically deviate by 0.5-1.5 percentage points from their
   historical mean).
...
10. **Rolling H in weight optimization (Draft 3 M2 fix, updated Draft 4 M1 fix):**
    Function 5 computes H_d as a rolling N_hist-day average for each training
    day d. This requires volume_history to extend at least N_hist + N_weight_train
    days before train_end_date (21 + 63 = 84 days). As of Draft 4, Function 9
    explicitly passes an extended slice (starting N_hist days before the first
    training day) to optimize_regime_weights, ensuring sufficient data. For the
    grid search context, the extended requirement is N_hist + (N_weight_train - 21)
    = 21 + 42 = 63 days before train_end_date. If insufficient history exists for
    the earliest training days (initial warm-up period only), compute the average
    from whatever history is available.
...
- Extended volume_history slice for optimize_regime_weights (N_hist pre-context)
- Scaled delta application: scale * (hist_pct + delta) instead of scale * hist_pct + delta
- Re-optimization of weights on full N_weight_train window after regime count selection
- Training-time mean surprise is exactly zero by construction (tautological property)
