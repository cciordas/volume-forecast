...
FUNCTION calibrate_adaptive_limits(
    ...
    # Set threshold to the median cumulative fraction at (switchoff_bin - 1)
    # so the switch-off activates approximately when expected
    IF switchoff_bin > 1 AND len(cum_fractions_by_bin[switchoff_bin - 1]) > 0:
        switchoff_threshold = median(cum_fractions_by_bin[switchoff_bin - 1])
    ELSE:
...
FUNCTION generate_model_a_training_forecasts(
    ...
    FOR d IN training_days:
        ...
        FOR i = 1 TO I:
            ...
            # Use median regime for simplicity (we don't have live cumulative
            # volume during historical reconstruction)
...
FUNCTION run_daily_prediction(
    ...
    # For live prediction, Model A forecasts are computed on the fly
    model_a_baseline_pct = None   # will be updated as bins complete
    ...
    FOR current_bin = 0 TO I-1:
        ...
        IF current_bin >= 1 AND current_bin < I:
            ...
            # For sophisticated variant: compute Model A-based baseline
            IF model_state.params.use_model_a_baseline:
                # Use current raw_forecasts to build percentage baseline
                total_forecast = sum(observed_volumes) + sum(
                    raw_forecasts[current_bin+1 : I+1])
                IF total_forecast > 0:
                    model_a_baseline_pct = []
                    FOR j = 1 TO I:
                        IF j <= current_bin:
                            model_a_baseline_pct.append(
                                observed_volumes[j] / total_forecast)
                        ELSE:
                            model_a_baseline_pct.append(
                                raw_forecasts[j] / total_forecast)
                ELSE:
...
The sophisticated variant live-prediction mechanism (building Model A-based
baseline percentages from current raw_forecasts) is Researcher inference —
the paper states the option of using Model A forecasts as the surprise baseline
(p.19) but does not specify the real-time integration procedure.
...
15. **Joint term constraint:** For every bin i, verify that
    interday_term_counts[i] + intraday_model.term_count < 11
    (= max_dual_arma_terms + 1). This is the "fewer than 11 terms"
...
9. Cross-validation for regime count and surprise lag selection.
...
27. Sophisticated variant: Model A-based surprise baseline procedure.
