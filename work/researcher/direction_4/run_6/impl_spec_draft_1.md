...
trading volume by combining three signal components — a rolling historical
average (H), an inter-day ARMA forecast (D), and an intraday ARMA forecast (A)
— through a regime-dependent dynamic weight overlay. **Model B** forecasts
next-bin volume percentages by applying a surprise-regression adjustment
(extending Humphery-Jenner 2011's "dynamic VWAP" framework) on top of a
historical volume percentage curve.

The models are loosely coupled: Model A's raw volume forecasts can optionally
provide the surprise signal that drives Model B, but Model B can also operate
independently using simple historical-average-based surprises.
...
(deviations of actual bin volumes from a baseline forecast), subject to deviation
constraints and a late-day switch-off rule. (Satish et al. 2014, pp.18-19,
referencing Humphery-Jenner 2011.)
...
- `predict_next() -> float`: One-step-ahead forecast for the next unobserved
  ...
  stored in the model. Used during live prediction.
...
- `make_state(observations) -> state`: (Intraday model only.) Creates a fresh
  ...
  mutating the model's stored state. Returns a state object.
...
FUNCTION fit_intraday_arma(volume_history, seasonal_factors, N_intraday_fit,
    ...
    FOR p = 0 TO p_max_intra:
        FOR q = 0 TO q_max_intra:
            # "fewer than 11 terms" constraint (Satish et al. p.18 para 1)
            # This applies to the combined dual ARMA; enforce as soft
            # constraint on each individual model.
            total_terms = p + q + 1
            IF total_terms > 10:
                CONTINUE
...
FUNCTION minimize_mape(records) -> weights[3]
    ...
    # Subject to: w_H, w_D, w_A >= 0 (non-negativity)
    #
    # Approach: use Nelder-Mead on exp-transformed parameters to ensure
    # non-negativity without explicit constraints.
    #   w_j = exp(theta_j), optimize over theta_j
    #
    # Multiple restarts (e.g., 5) with different random initializations
    # to avoid local minima. Return weights from best restart.

    def objective(theta):
        w = [exp(t) for t in theta]
        errors = []
    ...
    FOR restart = 1 TO N_optimizer_restarts:
        theta_init = [log(1/3) + random_normal(0, 0.5) FOR _ = 1 TO 3]
        result = nelder_mead(objective, theta_init, max_iter=1000)
        IF result.fun < best_obj:
            best_obj = result.fun
            best_w = [exp(t) FOR t IN result.x]
...
- Nelder-Mead with exp-transformation for non-negative weights.
- Evenly spaced percentile thresholds.
...
volume surprises (actual percentage minus historical percentage for each bin),
...
FUNCTION fit_surprise_regression(
    ...
    #   target: surprise[d, i] = actual_pct[d, i] - hist_pct_d[i]
    ...
    FOR d IN training_days:
        # Compute hist_pct as of day d (using days before d)
        hist_pct_d = compute_historical_percentages(
        ...
        # Compute surprises
        surprise_d = [actual_pct_d[i] - hist_pct_d[i] FOR i = 1 TO I]
    ...
    # (Mean surprise is zero by construction since hist_pct is the mean
    #  of actual_pct over the training window, so no intercept needed.)
...
equities. The no-intercept regression follows Satish et al. 2014, p.19 ("Note
that we perform both regressions without the inclusion of a constant term").
The training procedure (rolling surprise computation, lagged features) is
Researcher inference based on the described methodology. The observation that
mean surprise is zero by construction is Researcher inference.
...
FUNCTION predict_volume_percentage(
        ...
        deviation_limit,            # max allowed departure from hist_pct
    ...
    # Using the simple approach: assume remaining volume follows hist_pct
    ...
    FOR j = 1 TO current_bin:
        ...
        ELSE:
            actual_pct.append(hist_pct[j])

    # Compute surprises for observed bins
    surprises = [actual_pct[j] - hist_pct[j] FOR j = 1 TO current_bin]
    ...
    base_pct = hist_pct[next_bin]
    adjusted_pct = base_pct + delta

    # Scale for remaining fraction:
    # The hist_pct values assume full-day context. Scale by the fraction
...
reached, return to a historical approach"). The remaining-fraction scaling is
Researcher inference — the paper does not detail how conditional percentages
are computed from unconditional hist_pct, but this is the natural approach
to ensure percentages sum to 1 over remaining bins. The zero-padding for
early bins is Researcher inference.
...
FUNCTION train_full_model(
    ...
    # Step 3: Fit inter-day ARMA models
    vol_hist_interday = volume_history[train_end_date - N_interday_fit + 1 : train_end_date + 1]
    interday_models = fit_interday_arma(vol_hist_interday, params.N_interday_fit,
                                         params.p_max, params.q_max)

    # Step 4: Fit intraday ARMA model
...
FUNCTION run_daily_prediction(
    ...
    ) -> (raw_forecasts[1..I], pct_forecasts[2..I])
    ...
    pct_forecasts = array of length I-1
    ...
    FOR current_bin = 0 TO I-1:
        ...
        IF current_bin >= 1 AND current_bin < I:
            ...
            pct_forecasts[next_bin] = predict_volume_percentage(
                ...
                model_state.params.deviation_limit,
                model_state.params.switchoff_threshold)
...
Training Phase:
  volume_history ──┬── Function 1 ──> seasonal_factors[1..I]
                   ...
                   ├── Function 3 ──> interday_models[1..I]
                   ├── Function 4 ──> intraday_model
                   │   (uses seasonal_factors)
...
| regime_weights | [N_regimes, 3] | float >= 0 | per-regime [w_H, w_D, w_A] |
...
(2014): Model A with all three components + regime-switching weights, and
Model B with the surprise regression. This is the most complete variant
described in the paper and yields the best reported results.

Simpler variants could omit the intraday ARMA (using only H + D with regime
weights) or omit regime switching entirely (using a single weight set), but
these are strictly inferior per the paper's results. The full model is
selected because:
...
| q_max_intra | Max MA order for intraday ARMA | 4 | Medium — constrained by "fewer than 11 terms" | 2-5 |
| max_dual_arma_terms | Max total terms in dual ARMA model | 10 | Medium — paper says "fewer than 11" | 8-12 |
...
| N_surprise_lags | Number of lagged surprise terms | 5 | High — paper says "optimal number of model terms" found but not disclosed | 1-10 |
| deviation_limit | Max departure from historical VWAP curve | 0.10 (10%) | Medium — safety constraint | 0.05-0.20 |
| switchoff_threshold | Cumulative fraction for Model B switch-off | 0.80 (80%) | Medium — safety constraint | 0.70-0.90 |
...
- intraday_model: fitted on trailing ~21 days of deseasonalized data.
- regime_thresholds and regime_weights: computed by Function 5.
...
4. **Surprise lag selection:** Use expanding-window cross-validation over the
   surprise training window. For each candidate N_surprise_lags in {1,...,10},
   fit the regression on the first portion and evaluate MAE on the held-out
   portion. Select the lag count with lowest validation MAE.

5. **Evaluation:** Compute MAPE for Model A and MAE for Model B over the full
   out-of-sample period using Function 12.

6. **Re-estimation schedule:** In production, re-estimate ARMA models and
   weights monthly. Update hist_avg, hist_pct, and ARMA observation states
   daily. Seasonal factors can be updated weekly or monthly (they change
   slowly). (Satish et al. 2014, p.17: "We compute this model on a rolling
...
1. **Historical average baseline:** With all regime weights set to [1,0,0]
   (H only), Model A should reproduce the simple rolling average baseline.
   MAPE should match the "Historical" column in Exhibit 9 (~0.00874 for
   15-minute percentage error).

2. **Seasonal factors positive:** All seasonal_factors[i] > 0 after the
   zero-floor guard.

3. **Volume percentages sum to ~1:** hist_pct[1] + ... + hist_pct[I] should
   be approximately 1.0 (within floating-point tolerance).

4. **Deseasonalized mean ~1:** The mean of deseasonalized intraday values
   should be approximately 1.0 by construction (each bin divided by its
   mean over the deseasonalization window).

5. **ARMA stationarity:** All fitted ARMA models should have AR roots outside
   the unit circle. The enforce_stationarity flag ensures this.

6. **Regime assignment deterministic:** For the same cumulative volume and
   historical data, assign_regime should always return the same regime.

7. **Deviation clamp active:** On a stock with stable volume patterns,
   |delta| should rarely approach deviation_limit (0.10). Typical delta
   values should be in [-0.03, +0.03].

8. **Switch-off fires:** For a stock that reaches 80% of daily volume by
   bin ~21 (typical for U.S. equities), the switch-off should activate
   for the last ~5 bins.

9. **No-intercept mean-zero check:** At training time, the mean of all
   surprise values in the regression should be approximately zero (since
   hist_pct is the mean of actual_pct over the training window).

10. **Weight non-negativity:** All optimized weights w_H, w_D, w_A >= 0
    (guaranteed by exp-transformation).

11. **Fallback equivalence:** When all ARMA models are FALLBACK, Model A
    should degenerate to a simple weighted historical average (effectively
    V_hat = w_H * H + w_D * H + w_A * H = H).

12. **Monotonic improvement:** Adding the inter-day ARMA (H+D) should improve
    over H alone; adding the intraday ARMA (H+D+A) should improve further.
    If not, check ARMA fitting or weight optimization.

13. **Percentage forecast non-negative:** pct_hat >= 0 for all bins
...
2. **First bin of the day (bin 1):** No prior intraday observations.
   ...
   surprise computation). Use hist_pct[1] as default.
...
| Deviation limit (10%) | Satish et al. 2014, p.24 (referencing Humphery-Jenner 2011) |
| Switch-off (80%) | Satish et al. 2014, p.24 (referencing Humphery-Jenner 2011) |
...
| No-intercept regression | Satish et al. 2014, p.19 |
...
3. ARMA model interface (predict_next, append_observation, etc.).
4. Nelder-Mead optimizer with exp-transformation for weight optimization.
5. Multiple random restarts for the optimizer.
6. Evenly spaced percentile thresholds for regime boundaries.
7. Grid search over N_regimes in {1..5}.
8. Cross-validation for regime count and surprise lag selection.
9. Independent-segment (panel) ARMA fitting for intraday model.
10. Zero-padding for early bins in Model B feature construction.
11. Remaining-fraction scaling for conditional percentage forecasts.
12. Non-negativity floor on V_hat and pct_hat.
13. Daily orchestration structure (pre-market / intraday / end-of-day).
14. Re-estimation schedule (monthly for ARMA, daily for hist_avg).
15. Default regime assignment (median percentile) when no bins observed.
16. Winsorization suggestion for extreme volume days.
17. Estimated total daily volume approach in Function 9.
