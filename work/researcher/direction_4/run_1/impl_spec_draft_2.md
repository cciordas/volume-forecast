...
PROCEDURE ForecastVolumePercentage(symbol, day_t, current_bin, pct_models,
    ...
    raw_forecasts = ForecastRawVolume(symbol, day_t, current_bin=1, 
                                      raw_volume_model)
    raw_total = sum(raw_forecasts[k] for k in {1, ..., I})
    V_total_est = (
        sum(volume[s, day_t, j] for j in {1, ..., current_bin})  # observed
        + sum(raw_forecasts[k] for k in {current_bin + 1, ..., I})  # forecast
    )
    ...
    FOR k in {1, ..., K_reg}:
        ...
        expected_pct = raw_forecasts[j] / raw_total
    ...
    cum_pct = sum(volume[s, day_t, j] for j in {1,...,current_bin}) / \
              V_total_est
    ...
    #   p_hat = p_hist[next_bin] + delta (already clipped by deviation constraint)
    # When switch-off HAS triggered (delta = 0):
    #   p_hat = p_hist[next_bin], no further adjustment
    #
    # Renormalization:
    ...
    #
    # Specifically:
    #   remaining_fraction = 1.0 - sum(volume[s,t,j] for j in 1..current_bin)
    #                              / V_total_est
...
Dynamic Weight Combiner
  ...
  |      = sum(observed volumes) + sum(raw forecasts for remaining bins)
  |      Shape: scalar, float64
  |      UPDATE ORDERING: compute V_hat FIRST, then V_total_est
  |
  +---> Volume Surprise Computation
         expected_pct[j] = raw_forecast[j] / sum(raw_forecasts)
         ...
         Constraint Application
           (1) Clip: |delta| <= max_deviation * p_hist[next_bin]
           (2) Switch-off: if cum_pct >= 0.80, delta = 0
         Renormalization (scales p_hist baseline, not delta)
...
2. **MAPE reduction by time-of-day:** Error reduction should increase through the day, from roughly 10-15% at 9:30-10:00 to 25-35% at 15:00-15:30 (Satish et al. 2014, Exhibit 6, p.22). This is because the intraday ARMA component gains more information as the day progresses.
...
10. **Regime boundary instability:** Near percentile boundaries (e.g., 33rd percentile), small changes in cumulative volume could cause regime switches between adjacent bins. The min_regime_bins parameter (default 3) avoids early-day instability. For mid-day instability, implement hysteresis: once a regime is selected, require the percentile to cross the boundary by a margin (e.g., 5 percentile points) before switching.
