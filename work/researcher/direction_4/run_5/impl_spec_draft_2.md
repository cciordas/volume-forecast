...
**Critique assessment:** Draft 1 received 2 major, 4 moderate, and 5 minor issues. All are valid and addressed below.

| Issue | Severity | Resolution |
|-------|----------|------------|
| M1: Surprise baseline mismatch | Major | Unified to unconditional forecasts in both training and prediction (option a). Simpler, internally consistent. |
...
| m1: Inter-day leakage | Minor | Added acknowledgment note in Function 5. |
| m2: ClassifyRegime_Training | Minor | Replaced with direct call to ClassifyRegime with training-period distribution. |
| m3: Exhibit 6 description | Minor | Revised to accurately reflect non-monotonic pattern. |
| m4: Intraday ARMA day-boundary | Minor | Clarified per-segment likelihood as correct approach; concatenation as pragmatic approximation. |
| m5: No-intercept citation | Minor | Marked as Researcher inference with self-justifying reasoning. |
    ...
    FOR n_reg in regime_candidates:  # e.g., [3, 4, 5]
        ...
        FOR each day d in train_start..train_end:
            ...
            FOR each bin j in 1..I:
                # Use ClassifyRegime directly with training-period distribution
                observed_up_to_j = {k: volume_data[stock, bin=k, day=d]
                                    for k in 1..j}
                regime = ClassifyRegime(
                    observed_up_to_j, j, cumvol_dist, percentiles,
...
FUNCTION EvaluateWeights(volume_data, stock, val_start, val_end,
                          weights, cumvol_dist, percentiles, n_reg,
                          H_array, interday_models, intraday_model,
    ...
    FOR each day d in val_start..val_end:
        ...
        FOR each bin j in 1..I:
            ...
            # Classify regime using observed volumes up to bin j
            observed_up_to_j = {k: volume_data[stock, bin=k, day=d]
                                for k in 1..j}
            regime = ClassifyRegime(
                observed_up_to_j, j, cumvol_dist, percentiles,
