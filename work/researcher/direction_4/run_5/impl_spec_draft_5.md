...
**Critique 4 assessment:** Draft 4 received 0 major, 0 moderate, and 1 minor issue. The spec was declared implementation-ready. The single cosmetic issue is addressed below.

| Issue | Severity | Resolution |
|-------|----------|------------|
| m1: Residual "both MSE and MAPE" language in Function 5 docstring | Minor | Changed docstring to describe MSE as the primary objective with MAPE noted as a future enhancement, consistent with Function 5 pseudocode and Calibration section. |

### Prior Revisions (Draft 4)

| Issue | Severity | Resolution |
|-------|----------|------------|
| Mo1: predict_interday MA buffer reconstruction circularity | Moderate | Replaced undefined `model.predict_from_params()` and circular MA reconstruction with sequential burn-in processing from an extended historical window. MA buffers are now bootstrapped by processing (burn_in + q) days forward with AR+MA terms, starting from zeros. After burn_in steps the MA state converges for invertible processes. |
| m1: Calibration section MAPE variant still described as active | Minor | Aligned Calibration section text with Function 5 pseudocode: MAPE variant is now consistently described as a future enhancement, not an active switching mechanism. |
| m2: Day-index type ambiguity in range expressions | Minor | Added explicit convention note in Model Description: all date arithmetic uses trading-day indices. `is_full_trading_day()` filters half-day sessions, not weekends/holidays. |

### Prior Revisions (Draft 3)

| Issue | Severity | Resolution |
|-------|----------|------------|
| Mo1: ClassifyRegime training/prediction mismatch | Moderate | Changed Function 5 and EvaluateWeights to use bins 1..j-1 (causal) for regime classification, matching prediction-time behavior. |
| Mo2: Dead H_array parameter in EvaluateWeights | Moderate | Removed H_array from EvaluateWeights signature and calling code. Function already computes H via compute_H_asof(). |
| m1: Intraday ARMA conditioning state initialization | Minor | Clarified that conditioning initializes AR/MA buffers to zeros, matching per-segment likelihood training. Removed "post-training state" language. |
| m2: Exhibit 9 30-minute arithmetic inconsistency | Minor | Added footnote noting the paper's stated 2.95% does not match arithmetic from its own table values (~3.43%). |
| m3: MSE vs MAPE variant not fully specified | Minor | Removed MAPE variant from Function 5 pseudocode; retained as future enhancement note in Calibration section only. |
| m4: predict_interday state management | Minor | Added explicit buffer-update mechanism for training-time usage of predict_interday. |

### Prior Revisions (Draft 2)

| Issue | Severity | Resolution |
|-------|----------|------------|
| M1: Surprise baseline mismatch | Major | Unified to unconditional forecasts in both training and prediction. |
| M2: CV placeholder | Major | Full expanding-window CV pseudocode added to Function 8. |
| Mo1: Missing helper functions | Moderate | Added pseudocode for BuildCumVolDistribution, EvaluateWeights, compute_hist_pct. |
| Mo2: 11-term constraint | Moderate | Changed from soft guardrail to hard constraint with fallback and logging. |
| Mo3: N_hist for H vs hist_pct | Moderate | Explicitly shared with documented reasoning. |
| Mo4: Humphery-Jenner provenance | Moderate | Added provenance flags throughout Model B sections. |
| m1-m5 | Minor | Inter-day leakage note, ClassifyRegime_Training fix, Exhibit 6 description, day-boundary handling, no-intercept citation. |
...
**Provenance note on Model B:** Humphery-Jenner (2011) is not among the assigned papers. All Model B details are derived second-hand through Satish et al. (2014, pp.18-19, 24), who state they "successfully replicate[d] Humphery-Jenner's results" and "extended his work." Where Model B design decisions rest solely on Satish et al.'s description of Humphery-Jenner (rather than Satish et al.'s own verified extensions), this is flagged explicitly. The developer should understand that these portions cannot be independently verified against the original source.
...
**Date arithmetic convention:** All date arithmetic in this specification uses trading-day indices. The expression `day_d - N` refers to N trading days before day_d. Non-trading days (weekends, holidays) are not included in the index space. The `is_full_trading_day()` predicate filters half-day trading sessions (which have only 13 bins instead of 26) from full-day sessions; it does not filter weekends or holidays, which are already absent from the trading-day index.

### Pseudocode

The system is organized into 10 functions plus 6 helper functions: 5 main functions for Model A (training, state update, forecasting, regime classification, weight optimization), 3 for Model B (training, forecasting, evaluation), and 2 for orchestration (daily workflow, re-estimation scheduling). Helper functions (BuildCumVolDistribution, EvaluateWeights, compute_H_asof, predict_interday, predict_intraday_for_training, compute_hist_pct) are defined inline.
...
FUNCTION FitIntraDayARMA(volume_data, stock, reference_date, N_intraday_fit,
                          p_max_intra, q_max_intra, seasonal_factor,
    ...
    Implementation: use per-segment likelihood summation (approach 2). For
    each day-segment, initialize the ARMA state to zero (or the unconditional
    mean) at the start of the segment, compute the log-likelihood over that
    segment's I observations, and sum across all segments. This is the
    correct approach for truly independent segments.

    Pragmatic alternative: fit a single ARMA model on the concatenated series
    (ignoring day boundaries). This introduces a small bias at the first
    p observations of each day (where AR lags span the overnight gap), but
    the impact diminishes as N_intraday_fit grows (each boundary contributes
    p contaminated observations out of ~I*N_intraday_fit total). This is an
    acceptable simplification if per-segment likelihood is too complex to
    implement; log a note when using this approach.

    [Researcher inference: paper does not specify day-boundary handling.
     Per-segment likelihood is the theoretically correct approach.
     Concatenation is a pragmatic approximation.]
    ...
    FOR p in 0..p_max_intra:   # p_max_intra = 4 (AR < 5)
        FOR q in 0..q_max_intra:  # q_max_intra = 5
            ...
            TRY:
                # Fit with per-segment likelihood summation:
                # For each day-segment:
                #   1. Initialize AR buffer to zeros, MA buffer to zeros.
                #   2. For each observation in the segment:
                #      a. Compute prediction from AR buffer, MA buffer, coeffs.
                #      b. Compute residual = observation - prediction.
                #      c. Add log-likelihood contribution for this residual.
                #      d. Shift AR buffer (append observation).
                #      e. Shift MA buffer (append residual).
                #   3. Skip first p observations from likelihood sum
                #      (conditioning on initial values).
                # Sum log-likelihoods across all segments.
    ...
    # --- Hard constraint: combined dual ARMA terms < 11 ---
    # Reference: Satish et al. 2014, p.18, "we found that the autoregressive
    #   (AR) coefficients quickly decayed, so that we used AR lags with a
    #   value less than five. As a result, we fit each symbol with a dual
    #   ARMA model having fewer than 11 terms."
    # The passage reads as: because AR < 5, the combined term count is < 11.
    # We implement this as a hard constraint. If violated, reduce the
    # intraday model order (since it is a single model affecting all bins,
    # whereas inter-day models are per-bin and individually smaller).
    IF best_model is not None AND interday_models is not None:
        intra_terms = best_model.p + best_model.q + 1  # +1 for constant
        # Find the maximum inter-day term count across all bins
        max_interday_terms = 0
        FOR i in 1..I:
            IF interday_models[i] is not None:
                inter_terms_i = interday_models[i].p + interday_models[i].q + 1
                max_interday_terms = max(max_interday_terms, inter_terms_i)
        combined = intra_terms + max_interday_terms

        IF combined >= 11:
            LOG_WARNING(f"Combined terms {combined} >= 11; "
                        f"reducing intraday model order")
            # Re-fit with reduced order: cap p + q for intraday so that
            # intra_terms + max_interday_terms < 11
            max_intra_terms = 11 - max_interday_terms - 1  # -1 for safety
            IF max_intra_terms < 2:
                max_intra_terms = 2  # minimum: ARMA(1,0) or ARMA(0,1)

            # Re-do AICc search with tighter bounds
            best_aicc_reduced = +infinity
            best_model_reduced = None
            FOR p in 0..p_max_intra:
                FOR q in 0..q_max_intra:
                    IF p == 0 AND q == 0:
                        CONTINUE
                    k = p + q + 1
                    IF k > max_intra_terms:
                        CONTINUE  # skip: would violate combined constraint
                    n_eff = sum(max(len(seg) - p, 0) for seg in day_segments)
                    IF n_eff <= k + 1:
                        CONTINUE
                    TRY:
                        ...
                        aic = -2 * model.log_likelihood + 2 * k
                        aicc = aic + 2 * k * (k + 1) / (n_eff - k - 1)
                        IF aicc < best_aicc_reduced:
                            best_aicc_reduced = aicc
                            best_model_reduced = model
                    CATCH (ConvergenceError, LinAlgError):
                        CONTINUE
            IF best_model_reduced is not None:
                best_model = best_model_reduced
                LOG_INFO(f"Reduced intraday ARMA to "
                         f"({best_model.p},{best_model.q}), "
                         f"combined terms now "
                         f"{best_model.p + best_model.q + 1 + max_interday_terms}")
            ELSE:
                LOG_WARNING("All reduced-order models failed; "
                            "keeping original (constraint violated)")
...
FUNCTION ClassifyRegime(observed_volumes, current_bin, cumvol_distribution,
    ...
    # See BuildCumVolDistribution for how this is constructed.
...
FUNCTION OptimizeRegimeWeights(volume_data, stock, reference_date,
                                ...
                                seasonal_factor, regime_candidates) -> best_config:
    """
    ...
    [Researcher inference: the paper does not specify the loss function,
     ...
     MSE as the primary objective; a MAPE-based alternative is noted as a future enhancement.]

    Note on information leakage (m1): The inter-day ARMA parameters were
    estimated using data that includes the training days used here. For a
    126-day fitting window, each day contributes ~0.8% of the training data,
    so parameter sensitivity to any single day is small and the resulting
    "in-sample" forecast bias is negligible. A fully rigorous approach would
    use expanding-window ARMA re-estimation during training (at much higher
    computational cost). [Researcher inference]
    """
    ...
    FOR n_reg in regime_candidates:  # e.g., [3, 4, 5]
        ...
        FOR each day d in train_start..train_end:
            ...
            FOR each bin j in 1..I:
                # Classify regime using bins BEFORE j (causal: matching
                # prediction-time behavior in Function 6, which passes
                # current_bin - 1 to ClassifyRegime).
                # For j=1: empty observed_volumes, current_bin=0 -> hits
                # min_regime_bins guard -> returns default_regime.
                observed_before_j = {k: volume_data[stock, bin=k, day=d]
                                     for k in 1..j-1}
                regime = ClassifyRegime(
                    observed_before_j, j - 1, cumvol_dist, percentiles,
                    min_regime_bins, default_regime)
        ...
        FOR r in 0..n_reg-1:
            ...
            # Objective: minimize MSE with simplex constraint
            # w1 + w2 + w3 = 1, all >= 0
            # This is a constrained quadratic program with a closed-form
            # solution (non-negative least squares on the simplex).
            #
            # Note: MSE is used despite evaluating on MAPE (metric mismatch).
            # MSE is smooth, convex, and has a unique minimum, making it
            # reliable with constrained solvers (SLSQP/NNLS). A MAPE-based
            # alternative (variable transform w_raw = exp(w_log) with
            # Nelder-Mead) could be explored as a future enhancement -- see
            # Calibration section.
        ...
        val_mape = EvaluateWeights(
            ...
            interday_models, intraday_model, seasonal_factor)
...
##### Helper: BuildCumVolDistribution

```
FUNCTION BuildCumVolDistribution(volume_data, stock, start_date,
                                  end_date) -> cumvol_distribution[1..I]:
    """
    Build per-bin cumulative volume distributions for regime classification.
    For each bin index i, produces a sorted array of historical cumulative
    volumes (sum of bins 1 through i) observed across all training days.

    The distribution at bin i represents: "how much total volume had been
    traded by the end of bin i on each historical day?" This is used by
    ClassifyRegime to determine whether today's cumulative volume is
    unusually high or low relative to history.

    [Researcher inference: paper says "historical percentile of the observed
     cumulative volume" (p.18) but does not detail the distribution
     construction. Per-bin CDFs are the natural choice: the percentile rank
     of cumulative volume at bin 5 should be compared against other days'
     cumulative volumes at bin 5, not at other bin indices.]
    """
    ...
    FOR each bin i in 1..I:
        ...
        FOR each day d in start_date..end_date:
            IF NOT is_full_trading_day(d):
                CONTINUE
            # Cumulative volume from bin 1 through bin i
            cumvol = sum(volume_data[stock, bin=k, day=d] for k in 1..i)
            cumvols_at_bin_i.append(cumvol)

        # Sort for percentile lookup via bisect
        cumvol_distribution[i] = sorted(cumvols_at_bin_i)

    RETURN cumvol_distribution
```

##### Helper: EvaluateWeights

```
FUNCTION EvaluateWeights(volume_data, stock, val_start, val_end,
                          weights, cumvol_dist, percentiles, n_reg,
                          interday_models, intraday_model,
                          seasonal_factor) -> mape:
    """
    Evaluate a weight configuration on a validation set. Computes MAPE
    of the weighted combination forecast against actual volumes.

    [Researcher inference: evaluation procedure not specified in paper.
     Standard out-of-sample MAPE evaluation.]
    """

    abs_pct_errors = []

    FOR each day d in val_start..val_end:
        IF NOT is_full_trading_day(d):
            CONTINUE
        FOR each bin j in 1..I:
            actual = volume_data[stock, bin=j, day=d]
            IF actual < min_volume_floor:
                CONTINUE  # skip near-zero bins for MAPE stability

            # Classify regime using observed volumes BEFORE bin j (causal),
            # matching prediction-time behavior in Function 6.
            observed_before_j = {k: volume_data[stock, bin=k, day=d]
                                 for k in 1..j-1}
            regime = ClassifyRegime(
                observed_before_j, j - 1, cumvol_dist, percentiles,
    ...
    IF len(abs_pct_errors) == 0:
        RETURN +infinity
    RETURN mean(abs_pct_errors)
```

##### Helper: compute_H_asof
...
FUNCTION predict_interday(model, volume_data, stock, bin_i, day_d) -> float:
    ...
    Important: for training-time usage (Function 5 loop over multiple days),
    the model's AR/MA buffers must reflect observations through day d-1.
    The approach: use the model's fitted parameters (phi, theta, constant)
    with actual volume observations for bin i on days d-p through d-1 as
    AR inputs, and the corresponding residuals as MA inputs. This avoids
    needing to save/restore model state for each training day.

    [Researcher inference: the paper does not specify how inter-day
     forecasts are produced during training. We use direct buffer
     reconstruction from historical data to ensure correct causal ordering.]
    """
    IF model is None:
        RETURN compute_H_asof(volume_data, stock, bin_i, day_d, N_hist)

    # Reconstruct AR/MA buffers from historical data through day d-1
    p = model.order_ar
    q = model.order_ma

    # AR buffer: actual volumes for bin i on days d-p through d-1
    ar_history = [volume_data[stock, bin=bin_i, day=d_prev]
                  for d_prev in (day_d - p)..(day_d - 1)
                  if is_full_trading_day(d_prev)]

    # MA buffer: reconstruct by sequential burn-in processing.
    # For ARMA(p,q) models, MA residuals depend on prior residuals,
    # creating a recursive dependency. We resolve this by processing
    # a burn-in window forward from zeros: after burn_in steps the
    # MA state converges to the true state (burn-in artifact decays
    # exponentially for invertible MA processes).
    #
    # [Researcher inference: the paper does not describe MA buffer
    #  reconstruction. Sequential burn-in is standard practice for
    #  initializing ARMA state from a cold start.]
    burn_in = max(2 * q, 10)
    ma_buf = [0.0] * q  # initialize MA buffer to zeros

    # Process (burn_in + q) trading days ending at day_d - 1
    start_day = day_d - (burn_in + q)
    FOR d_prev in start_day..(day_d - 1):
        IF NOT is_full_trading_day(d_prev): CONTINUE
        actual_prev = volume_data[stock, bin=bin_i, day=d_prev]

        # Compute one-step-ahead prediction at d_prev using current buffers
        pred = model.constant
        ar_vals = [volume_data[stock, bin=bin_i, day=d_ar]
                   for d_ar in (d_prev - p)..(d_prev - 1)
                   if is_full_trading_day(d_ar)]
        FOR k, phi_k in enumerate(model.phi):
            IF k < len(ar_vals):
                pred += phi_k * ar_vals[-(k+1)]
        FOR k, theta_k in enumerate(model.theta):
            IF k < len(ma_buf):
                pred += theta_k * ma_buf[-(k+1)]

        resid = actual_prev - pred
        ma_buf.append(resid)
        IF len(ma_buf) > q:
            ma_buf.pop(0)  # keep only q most recent residuals

    ma_history = ma_buf

    # One-step-ahead forecast using reconstructed buffers
    forecast = model.constant
    FOR k, phi_k in enumerate(model.phi):
        IF k < len(ar_history):
            forecast += phi_k * ar_history[-(k+1)]
    FOR k, theta_k in enumerate(model.theta):
        IF k < len(ma_history):
            forecast += theta_k * ma_history[-(k+1)]

    RETURN forecast
```

##### Helper: predict_intraday_for_training
...
##### Helper: compute_hist_pct

```
FUNCTION compute_hist_pct(volume_data, stock, reference_date, N_hist) -> hist_pct[1..I]:
    """
    Compute historical average volume percentage per bin. For each bin i,
    compute the average fraction of daily volume occurring in bin i over
    the trailing N_hist days.

    N_hist is shared with the raw volume historical average H. Both serve
    as "recent history" baselines and benefit from the same window length:
    a window that is responsive enough to capture regime shifts but long
    enough to be stable. Using separate windows would add a parameter
    without clear empirical guidance from the paper.

    [Researcher inference: the paper does not explicitly state whether the
     same N applies to both raw and percentage historical averages. We use
     the same N_hist for both, following the principle of parsimony.
     N_hist appears only once in the paper (Exhibit 1 annotation, p.18)
     with no indication of separate values for different uses.]

    Reference: Satish et al. 2014, p.16, "Historical Window Average/Rolling
      Means... the same technique applies to constructing volume percentage
      bins."
    """

    bin_pcts = {i: [] for i in 1..I}

    FOR each day d in (reference_date - N_hist + 1)..reference_date:
        IF NOT is_full_trading_day(d):
            CONTINUE
        ...
        IF daily_total <= 0:
            CONTINUE
        FOR each bin i in 1..I:
            bin_pcts[i].append(volume_data[stock, bin=i, day=d] / daily_total)

    hist_pct = {}
    FOR each bin i in 1..I:
        IF len(bin_pcts[i]) > 0:
            hist_pct[i] = mean(bin_pcts[i])
        ELSE:
            hist_pct[i] = 1.0 / I  # uniform fallback
...
FUNCTION ForecastRawVolume(model_a, stock, date, current_bin,
    ...
    IF model_a.intraday_model is not None AND len(observed_deseas) > 0:
        ...
        # Implementation: initialize AR/MA buffers to zeros (matching
        # the per-segment likelihood training assumption that each day
        # starts with no overnight carryover -- see Function 3),
...
FUNCTION ForecastVolumePercentage(model_b, model_a, stock, date,
    ...
    Provenance: The dynamic VWAP framework, deviation bounds (10%), and
    switch-off mechanism (80%) originate from Humphery-Jenner (2011),
    described second-hand through Satish et al. (2014, pp.18-19, 24).
    Satish et al.'s own extension is the use of Model A's forecasts as
    the surprise baseline (p.19). The surprise regression structure is
    Satish et al.'s contribution; specific details like the rolling
    regression form and lag selection come from their description.
    """
    ...
    #   approach"
    # [Provenance: Humphery-Jenner (2011), described second-hand via
    #  Satish et al. p.24. Cannot verify original specification.]
    ...
    # Use Model A's UNCONDITIONAL forecasts (market-open, no observed bins)
    # as the surprise baseline. This ensures consistency between training
    # and prediction: both use the same unconditional baseline.
    #
    # Reference: Satish et al. 2014, p.18, "departures from a historical
    #   average approach, or a naive volume forecast model"
    # Reference: Satish et al. 2014, p.19, "we could apply our more
    #   extensive volume forecasting model" as the surprise base.
    # [Researcher inference: the unconditional (market-open) forecast is
    #  the "naive" forecast for the day. Using unconditional forecasts
    #  rather than conditioned ones ensures the surprise baseline is the
    #  same whether computed at training time or prediction time, avoiding
    #  the systematic mismatch where conditioned forecasts are more
    #  accurate and produce smaller surprises than unconditional ones.
    #  This is the simpler of two valid approaches (see M1 resolution).]
    ...
    # [Provenance: Humphery-Jenner (2011), described second-hand via
    #  Satish et al. p.24. The paper also states "we developed a separate
    #  method for computing the deviation bounds" (p.19), meaning the
    #  production system used adaptive/proprietary bounds, NOT the fixed
    #  10%. The 10% is a simplification from Humphery-Jenner's original.
    #  max_deviation is a primary tuning candidate.]
...
FUNCTION TrainPercentageRegression(volume_data, stock, reference_date,
    ...
    For each training day, compute Model A's UNCONDITIONAL forecasts
    (market-open, no observed bins) and derive surprises, then regress
    percentage deviations on lagged surprises.

    Uses unconditional forecasts as the surprise baseline for BOTH
    training and prediction, ensuring internal consistency (see M1
    resolution). The unconditional forecast represents the "naive"
    model's prediction at market open, and surprises measure how
    reality departed from that naive expectation.
    ...
    #   - The surprise at that bin (using UNCONDITIONAL Model A forecasts)
    #   - The percentage deviation from historical (actual_pct - hist_pct)

    # Collect training samples organized by day for proper CV splitting
    day_data = []  # list of (day_index, list of (x_val, y_val) pairs)
    ...
    FOR each day d in (reference_date - N_regression_fit + 1)..reference_date:
        ...
        # Compute unconditional Model A forecasts for this day
        # (market-open forecast, no observed bins)
        raw_fcst_unconditional = ForecastRawVolume(
            model_a, stock, d, current_bin=1, observed_volumes={})

        # Pre-compute surprises for all bins of this day
        day_surprises = {}
        FOR j in 1..I:
            expected = raw_fcst_unconditional[j]
            actual = volume_data[stock, bin=j, day=d]
        ...
        FOR j in 2..I:
            ...
            day_pairs.append((x_val, y_val))

        day_data.append((d, day_pairs))

    # --- Step 2: Select optimal number of lag terms via expanding-window CV ---
    ...
    #  the selection method. Time-series expanding-window CV is standard
    #  and respects temporal ordering.]

    n_days = len(day_data)
    min_train_days = 21  # minimum training days before first validation fold
    ...
    FOR L in 1..L_max:
        ...
        # Expanding-window CV with 1-day step:
        # Train on days 0..k, validate on day k+1
        FOR k in min_train_days..(n_days - 2):
            # Training set: all (X, y) pairs from days 0..k
            train_X = []
            train_y = []
            FOR day_idx in 0..k:
                FOR (x_val, y_val) in day_data[day_idx][1]:  # [1] = pairs
                    train_X.append(x_val[0:L])  # first L lags only
                    train_y.append(y_val)

            # Fit OLS without intercept on training set
            X_train = array(train_X)  # shape: (n_train_samples, L)
            y_train = array(train_y)  # shape: (n_train_samples,)

            IF len(train_X) < L + 1:
                CONTINUE  # insufficient samples for regression

            # beta_train = (X^T X)^{-1} X^T y (via pseudoinverse)
            beta_train = ols_no_intercept(X_train, y_train)

            # Validation set: all (X, y) pairs from day k+1
            val_pairs = day_data[k + 1][1]
            day_abs_errors = []
            FOR (x_val, y_val) in val_pairs:
                x_L = array(x_val[0:L])  # first L lags
                predicted_delta = dot(beta_train, x_L)
                day_abs_errors.append(abs(predicted_delta - y_val))

            IF len(day_abs_errors) > 0:
                cv_errors.append(mean(day_abs_errors))
    ...
    # [Researcher inference: the no-intercept decision for the surprise
    #  regression is based on the principle that when all surprises are
    #  zero (model predictions match reality), the adjustment delta
    #  should be zero. This is a natural constraint for a surprise-based
    #  correction model and is self-justifying. The paper's p.19 quote
    #  ("we perform both regressions without the inclusion of a constant
    #  term") refers to the VWAP-error validation regressions (Exhibits
    #  2-5), not directly to the surprise regression. However, the
    #  underlying reasoning (zero error implies zero correction) applies
    #  equally here.]
    all_X = []
    all_y = []
    FOR (d, pairs) in day_data:
        FOR (x_val, y_val) in pairs:
            all_X.append(x_val[0:best_L])
            all_y.append(y_val)

    X_final = array(all_X)  # shape: (n_samples, best_L)
    y_final = array(all_y)  # shape: (n_samples,)
    beta = ols_no_intercept(X_final, y_final)
...
FUNCTION ReEstimationSchedule(stock, current_date, model_a, model_b,
    ...
    model_a.intraday_model = FitIntraDayARMA(
        ...
        model_a.interday_models)

    # hist_pct for Model B also updated daily, using the same N_hist
    # as the raw volume historical average H.
    # [See compute_hist_pct for rationale on sharing N_hist.]
    model_b.hist_pct = compute_hist_pct(
        volume_data, stock, current_date - 1, N_hist)
    ...
    IF trading_days_since(last_refit_dates['regime']) >= 21:
        model_a.regime_config = OptimizeRegimeWeights(
            ...
            model_a.seasonal_factor, regime_candidates)
...
[Weighted Combination] -- V_hat[i] = w1*H[i] + w2*D[i] + w3*A[i]
  ...
  Model A Output: V_hat[i] for i in current_bin..I
       ...
       |             ^                  (unconditional baseline, same in train and predict)
       |             |
       |      [Unconditional Forecast] -- ForecastRawVolume(current_bin=1, observed={})
       |
       v
...
| N_hist | Days for historical rolling average (Component 1) and hist_pct (Model B) | 21 | Medium-High | [10, 60] |
...
| max_combined_terms | Hard constraint on combined inter-day + intraday ARMA terms | 11 | Low | [9, 13] |
...
| min_cv_train_days | Minimum training days for first CV fold (surprise regression) | 21 | Low | [10, 30] |
...
- **N_hist = 21:** Satish et al. 2014, Exhibit 1 caption, "Prior 21 days." Note: this appears in the diagram annotation, not the methodology text. The methodology (p.16) introduces N as "a variable that we shall call N" without disclosing its value. The 21-day value may be illustrative. Sensitivity upgraded to Medium-High to encourage testing alternatives. N_hist is shared between Model A's historical average (H) and Model B's historical percentage curve (hist_pct) -- see compute_hist_pct for rationale.
...
- **max_combined_terms = 11:** Satish et al. 2014, p.18, "we fit each symbol with a dual ARMA model having fewer than 11 terms." Implemented as a hard constraint: after independent model selection, check if combined terms (max inter-day + intraday) < 11; if not, reduce the intraday model order. See Function 3 for enforcement logic.
...
- **max_deviation = 0.10:** Satish et al. 2014, p.24, "depart no more than 10% away from a historical VWAP curve," referencing Humphery-Jenner (2011). [Provenance: Humphery-Jenner, second-hand.] However, the paper also states "we developed a separate method for computing the deviation bounds" (p.19), meaning the production system used adaptive/proprietary bounds, NOT the fixed 10%. The 10% is a simplification; max_deviation is a primary tuning candidate.
- **pct_switchoff = 0.80:** Satish et al. 2014, p.24, "once 80% of the day's volume is reached, return to a historical approach," referencing Humphery-Jenner (2011). [Provenance: Humphery-Jenner, second-hand.]
...
- **min_cv_train_days = 21:** [Researcher inference] Minimum training days for the first expanding-window CV fold. 21 days provides ~525 training samples for the regression, sufficient for stable OLS with up to 5 regressors.
...
3. **Historical averages:** Compute H[i] from trailing N_hist = 21 days for each bin. Compute hist_pct[i] using the same N_hist window (see compute_hist_pct for rationale on sharing the window).
...
5. **Intraday ARMA:** FitIntraDayARMA: single model on deseasonalized intraday volume from trailing N_intraday_fit = 21 days, with per-segment likelihood at day boundaries (or concatenation as a pragmatic alternative). Enforce the combined terms < 11 hard constraint against the inter-day models. Save the post-training AR/MA buffer state for use in condition() reset-and-reprocess.
...
7. **Percentage regression:** TrainPercentageRegression: compute unconditional Model A forecasts for trailing N_regression_fit = 63 days, derive surprises, fit OLS (no intercept) with expanding-window time-series cross-validated lag count (minimum 21 training days per fold, MAE error metric).
...
1. **ARMA model selection:** Automatic via AICc grid search over (p, q) combinations. No manual tuning required. Combined term count enforced as hard constraint (< 11). [Satish et al. 2014, p.17-18]
...
3. **Component weights:** Per-regime optimization minimizing MSE with simplex constraint (w1+w2+w3=1, all >= 0), using scipy SLSQP. Variant (future enhancement): MAPE minimization with variable transformation (w_raw = exp(w_log)) and Nelder-Mead could be explored as an alternative. If implemented, switch to MAPE variant only if it produces lower out-of-sample MAPE. [Researcher inference; paper says "minimizes the error" (p.18) but does not specify loss function or optimizer]

4. **Surprise regression lag count:** Expanding-window time-series cross-validation over L in {1, 2, ..., L_max}. Minimum training period: 21 days. Each fold trains OLS on days 1..k, evaluates MAE on day k+1's pairs. Select L minimizing average MAE across all validation folds. [Researcher inference; paper says they "identified the optimal number of model terms" (p.19) but does not disclose it or the selection method]
...
- Error reduction varies by time of day: lowest at market open (~10-12% at 9:30) and generally increasing toward close (~30-33% at 15:30), though with significant bin-to-bin variation in the median profile. The bottom-95% profile shows a smoother increasing trend from ~13% to ~38%. [Satish et al. 2014, Exhibit 6, p.22]
...
- 5-minute bins: 2.25% median reduction. 30-minute bins: 2.95% median reduction (as stated in the paper; note: the arithmetic from Exhibit 9's own HVWAP=0.0143 and DVWAP=0.01381 yields ~3.43%, suggesting a rounding or transcription error in the original paper). 15-minute bins show largest improvement. [Satish et al. 2014, Exhibit 9, p.23]
...
5. **Combined term count:** For each stock, compute (max interday terms across bins) + (intraday terms). Must be fewer than 11 (hard constraint). If the constraint activates frequently (> 20% of stocks), consider tightening p_max_intraday. [Satish et al. 2014, p.18]
...
18. **Train/predict surprise baseline consistency:** Verify that both Function 7 (prediction) and Function 8 (training) use unconditional Model A forecasts (current_bin=1, observed_volumes={}) as the surprise baseline. Any deviation from this creates the M1 mismatch.
...
13. **Combined term constraint activation:** When the 11-term hard constraint activates, the intraday model order is reduced. This may slightly increase intraday forecast error for bins where the full model would have been better, but prevents overfitting of the combined dual ARMA system. The constraint is expected to activate rarely (< 5% of stocks) when p_max_intraday = 4. [Researcher inference]
...
10. **Model B provenance gap:** Humphery-Jenner (2011) is not among the assigned papers. All Model B design details -- including the rolling regression structure, deviation bounds, and switch-off mechanism -- are derived second-hand through Satish et al.'s description (pp.18-19, 24). Satish et al. state they "extended his work" (p.19), meaning some details may differ from Humphery-Jenner's original. The developer cannot independently verify whether the spec's interpolations are consistent with the original. Key unverifiable decisions: the specific surprise formula, whether the regression uses an intercept, and the exact switch-off behavior. [Researcher inference]
...
| N_hist shared for H and hist_pct | Researcher inference (see compute_hist_pct and Parameters section) |
...
| Combined terms < 11 (hard constraint) | Satish et al. 2014, p.18, "As a result, we fit each symbol with a dual ARMA model having fewer than 11 terms" |
...
| Volume percentage methodology | Satish et al. 2014, pp.18-19; Humphery-Jenner (2011) (second-hand) |
| Model A as surprise baseline | Satish et al. 2014, p.19, "we could apply our more extensive volume forecasting model" |
| Unconditional surprise baseline (train+predict) | Researcher inference (M1 resolution; consistent with "naive volume forecast model," p.18) |
| Proprietary deviation bounds | Satish et al. 2014, p.19, "we developed a separate method for computing the deviation bounds" |
| No-intercept surprise regression | Researcher inference (zero surprise implies zero correction; self-justifying) |
| No-intercept VWAP-error regressions | Satish et al. 2014, p.19, "we perform both regressions without the inclusion of a constant term" |
...
| Deviation limit (10%) | Satish et al. 2014, p.24; Humphery-Jenner (2011) (second-hand) |
| Switch-off (80%) | Satish et al. 2014, p.24; Humphery-Jenner (2011) (second-hand) |
...
| Day-boundary handling for intraday ARMA | Researcher inference (per-segment likelihood recommended) |
...
| BuildCumVolDistribution | Researcher inference |
| EvaluateWeights | Researcher inference |
| compute_hist_pct | Researcher inference (with p.16 reference for methodology) |
| Expanding-window CV for lag selection | Researcher inference |
