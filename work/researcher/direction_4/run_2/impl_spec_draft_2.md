...
FUNCTION build_raw_volume_model(stock, date, historical_data):
    ...
    FOR each bin i in 1..I:
        ...
        FOR p in 0..5:
            FOR q in 0..5:
                ...
                # Reserve at least 2 terms for intraday (p=1,q=0 + constant)
                # so inter-day can use at most 8 terms
                IF p + q + 1 > 8: CONTINUE
    ...
    # Use the maximum interday_terms across all bins as the binding constraint
...
1. Each trading day before market open:
   ...
   c. Refit inter-day ARMA(p,q) models for each bin using AICc selection on the trailing N_interday_fit days. The joint term constraint (inter-day + intraday < 11 terms) must be enforced during model selection. [Satish et al. 2014, pp. 17-18]
...
2. During the trading day:
   a. After each bin observation, feed the new deseasonalized observation into the intraday ARMA and predict the next bin. The ARMA uses up to 4 prior bins as AR context (from Exhibit 1, "4 Bins Prior to Current Bin"). [Satish et al. 2014, Exhibit 1, p. 18]
...
2. **Insufficient history:** If fewer than N_hist days of data are available, fall back to whatever is available. If fewer than 6 months for seasonal factors, use all available data. Minimum viable: 63 trading days for seasonal factors, N_interday_fit days for inter-day ARMA. [Researcher inference]
