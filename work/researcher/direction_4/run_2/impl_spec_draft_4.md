...
FUNCTION predict_volume_percentage(stock, date, bin_j, raw_model):
    ...
    IF j + 1 < I:
        # Guard: when j+1 == I (last bin), there are no remaining bins to scale,
        # and the denominator would be zero. Skip redistribution in that case.
    ...
    # When j + 1 == I, pct_forecast[I] = adjusted_pct and no redistribution is needed.
...
| N_interday_predict | Number of recent daily observations used as ARMA lag inputs for prediction | 5 days [Exhibit 1, p. 18, "Prior 5 days" label for ARMA Daily] | Low -- determined by AR order p | p minimum; 5 is a safe default covering all candidate AR orders (1-5) |
