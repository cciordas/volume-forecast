...
FUNCTION select_num_factors(X_centered, N, P):
    ...
    for r_candidate in range(1, r_max + 1):
        # Estimate r_candidate-factor model using SVD on X_centered
...
**Implementation note:** The SVD in `select_num_factors` only needs to be
computed once; the same decomposition can be reused for all r_candidate values
by simply varying the number of singular vectors retained. Compute the thin SVD
once and iterate over truncation levels.
