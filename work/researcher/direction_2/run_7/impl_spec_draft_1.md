...
common factor structure (typically 1-3 factors) that can be extracted by PCA. The
remaining stock-specific residual exhibits short-memory serial correlation
...
FUNCTION extract_factors(X, r_max=10):
    ...
    r = best_r  # typically 1-3 for intraday volume data
...
FUNCTION fit_setar(e_series, n_grid=100):
    ...
    NOTE on notation ordering: BDF 2008 Eq. (11) writes the I(e <= tau) regime
    first with coefficients (phi_11, phi_12), and the I(e > tau) regime second
    with (phi_21, phi_22). Szucs 2017 Eq. (6) uses the SAME ordering. When
    implementing, be consistent: regime 1 = below threshold, regime 2 = above.
    """
    y = e_series[1:]
    x_lag = e_series[:-1]
    T = len(y)
    ...
    # Step 3.5: Final fit at best threshold
...
FUNCTION dynamic_vwap_execution(c_forecast, ar_params_or_setar_params,
    ...
    # Step 4.3: As each bin j_obs completes, revise remaining forecasts
    FOR j_obs = 0, 1, ..., k-2:
        ...
        # Execute: trade (revised_proportions[j_obs+1, i] * remaining_quantity[i])
        #          shares in the next bin
```

#### Top-Level Daily Pipeline

```
FUNCTION daily_pipeline(historical_data, day_to_forecast, N, k, L, model_type):
    """
    ...
    # Step 3: Fit specific component model per stock
    FOR each stock i:
        e_series_i = e_hat[:, i]   # shape (P,)
        IF model_type == "AR1":
            psi_1[i], psi_2[i], _ = fit_ar1(e_series_i)
        ELSE:  # "SETAR"
            phi_11[i], phi_12[i], phi_21[i], phi_22[i], tau[i], _ = \
                fit_setar(e_series_i)
    ...
    dynamic_vwap_execution(c_forecast, params, e_last, k, model_type)
```
...
**Type details:**
- All turnover values are float64 (small positive numbers, typically 1e-4 to 1e-1).
...
**Implemented variant:** Dynamic PCA-SETAR (primary) with AR(1) as secondary.

The SETAR model is preferred because:
1. BDF 2008 Table 2: SETAR achieves lower MAPE (0.0752 vs 0.0829 for ARMA) for
   volume prediction and lower VWAP tracking error (0.0706 vs 0.0772 in-sample).
   Reference: BDF 2008, Section 3.2, Table 2.
2. BDF 2008 out-of-sample: SETAR outperforms ARMA for 36 of 39 stocks.
   Reference: BDF 2008, Section 4.3.2.
3. Szucs 2017 Table: BDF_SETAR has lowest MAPE (0.399) across 33 DJIA stocks
   over 11 years. Reference: Szucs 2017, Section 5, Table (full sample results).
4. Computational cost: SETAR grid search adds negligible overhead over AR(1).

The AR(1) variant should also be implemented for comparison and as a fallback
for cases where SETAR estimation is unreliable (too few observations in a regime).
...
| L | Rolling estimation window in trading days | 20 | Moderate — shorter windows adapt faster but increase noise; longer windows smooth but may miss regime changes | 10-60 |
| N | Number of stocks in the cross-section | 30+ | Low-moderate — larger N improves factor estimation consistency per Bai (2003), but model works with as few as ~30 stocks | 20-500 |
| r | Number of common factors | Data-driven via IC_p2 | Moderate — too few factors leave common variation in residuals; too many overfit | 1-5 (typically 1-3) |
...
1. **Volume prediction accuracy (turnover MAPE):**
   - BDF_AR: approximately 0.40 MAPE across US large-cap stocks.
   - BDF_SETAR: approximately 0.40 MAPE (slightly lower than AR).
   - Benchmark U-method: approximately 0.50 MAPE.
   - This represents roughly a 20% MAPE reduction over the naive benchmark.
   Source: Szucs 2017, Section 5, full-sample results table (33 DJIA stocks,
   11 years).

2. **VWAP tracking error (out-of-sample):**
   ...
   Source: BDF 2008, Section 4.3.2, Table 2 (out-of-sample panel, 39 CAC40 stocks).

3. **Common component shape:** The common component forecast should exhibit the
...
4. **Specific component ACF:** The autocorrelation of the specific component
...
5. **Factor count:** For liquid equity markets, IC_p2 should typically select
   r = 1 to 3 factors. Source: BDF 2008, Section 2.2 (French market data).

6. **Factor normalization check:** After extraction, verify F_hat.T @ F_hat / P
...
5. **SETAR regime balance:** Both regimes should have a meaningful number of
   observations. If either regime has fewer than min_regime_obs data points, fall
   back to AR(1). Reference: Researcher inference — standard SETAR practice.
...
| Volume prediction accuracy (MAPE) | Szucs 2017, Section 5, full-sample results |
| VWAP tracking error results | BDF 2008, Section 4.3, Tables 2-7 |
| SETAR outperforms AR in most stocks | BDF 2008, Section 3.2; Szucs 2017, Section 5 |
| Estimation window = 20 days, re-estimated daily | BDF 2008, Section 3.1; Szucs 2017, Section 3 |
