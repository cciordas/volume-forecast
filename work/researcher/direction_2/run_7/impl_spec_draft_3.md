...
FUNCTION fit_ar1(e_series):
    ...
    - BDF 2008 Eq. (10) writes: psi_1 * e_{t-1} + psi_2 — AR coefficient
      FIRST, intercept SECOND.
    - Szucs 2017 Eq. (5) writes: c + theta_1 * e_{p-1} — intercept FIRST,
      AR coefficient SECOND.
    Parameter name mapping (this implementation follows BDF ordering):
      psi_1 (AR coefficient) = Szucs' theta_1
      psi_2 (intercept)      = Szucs' c
    """
...
FUNCTION daily_pipeline(historical_data, day_to_forecast, N, k, L):
    ...
    # (BDF 2008, Section 3.2) and 30 of 33 stocks in Szucs 2017
    # (Table 2c, MAPE pairwise comparison: BDF_SETAR vs BDF_AR).
    ...
    FOR each stock i:
        ...
        ELSE:
            ...
            # NOTE (Researcher inference): Because SETAR has 5 free parameters
            # (phi_11, phi_12, phi_21, phi_22, tau) versus AR(1)'s 2 (psi_1,
            # psi_2), SETAR's residual variance will almost always be lower than
            # AR(1)'s on the same training data. This rule is therefore
            # effectively "always use SETAR unless it catastrophically fails."
            # This is a defensible default given that SETAR consistently
            # outperforms AR(1) in both BDF and Szucs datasets. For genuine
            # model selection, replace residual variance with BIC:
            #   BIC_ar    = T * ln(sigma2_ar)    + 2 * ln(T)
            #   BIC_setar = T * ln(sigma2_setar) + 5 * ln(T)
            # and select the model with lower BIC. Neither paper provides a
            # model selection procedure, so either approach is valid.
...
hold:
  ...
  (b) SETAR residual variance exceeds AR(1) residual variance.
...
side by side. NOTE: Because SETAR has more free parameters (5 vs 2), the
residual variance rule is effectively "always use SETAR unless it
catastrophically fails" — see the NOTE in the pseudocode (Phase 3, model
selection) for discussion and a BIC-based alternative. Both approaches are
valid since the papers consistently show SETAR outperforming AR(1).

The SETAR model is preferred because:
1. BDF 2008 Table 2: SETAR achieves lower in-sample volume prediction MAPE
   (0.0752 vs 0.0829 for ARMA; Table 2 first panel, Section 3.2) and lower
   in-sample VWAP tracking error (0.0706 vs 0.0772; Table 2 second panel,
   Section 4.3.1). In-sample period: September 2 to December 16, 2003.
...
**VWAP tracking error (out-of-sample, BDF data):**
   ...
   Source: BDF 2008, Section 4.3.3, Table 2 (third panel, out-of-sample VWAP
   execution cost, portfolio of 39 CAC40 stocks).
...
| AR(1) notation difference between BDF and Szucs | BDF 2008, Eq. (10) vs Szucs 2017, Eq. (5) — opposite intercept/slope ordering |
...
| In-sample volume prediction MAPE | BDF 2008, Section 3.2, Table 2 (first panel, Sep 2 to Dec 16, 2003) |
| In-sample VWAP tracking error | BDF 2008, Section 4.3.1, Table 2 (second panel) |
| VWAP tracking error results (out-of-sample, portfolio) | BDF 2008, Section 4.3.3, Table 2 (third panel) |
| VWAP tracking error results (out-of-sample, per-stock) | BDF 2008, Section 4.3.2, Tables 4-7 |
...
| Model selection procedure (AR1 vs SETAR) | Researcher inference — BDF 2008 reports both without selection rule; residual variance rule is effectively "always SETAR"; BIC alternative offered |
