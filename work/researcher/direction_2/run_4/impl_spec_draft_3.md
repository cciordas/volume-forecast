...
Critique 2 raised 0 major and 3 minor issues. All are addressed in this revision:

- **m1-new (forecasts_history)**: Added `forecasts_history` construction in the dynamic
  execution loop body. The list accumulates per-stock turnover forecasts at each step,
  enabling downstream MAPE validation against actuals.
- **m2-new (portfolio MAPE)**: Added `compute_portfolio_mape` function in Group 5
  validation utilities. Computes weighted-average turnover forecast/actual across stocks
  per bin, then MAPE on the aggregate — matching BDF 2008 Table 2 methodology.
- **m3-new (VWAP cost scope)**: Added explicit note in the VWAP execution cost
  validation section stating that these benchmarks require trade-level price data
  beyond the scope of this volume model. Primary validation targets are volume forecast
  MAPE and MSE.

### Draft 2 Revision Notes (retained for traceability)

Critique 1 raised 3 major and 7 minor issues. All addressed in draft 2:
M1 (TSO vs. float), M2 (ARMA vs. AR), M3 (dynamic execution explanation),
m1 (SVD unification), m2 (variance DoF), m3 (SETAR notation), m4 (MAPE computation),
m5 (overnight transition), m6 (stationarity check), m7 (day_index precondition).
...
The implementation consists of 14 functions organized in five groups: data
...
function run_dynamic_execution(X_estimation, shares_data, k, L_days,
    ...
    Returns:
        ...
        forecasts_history: list of length k, each entry is array of shape
                           (k - j, N) — turnover forecasts for remaining bins
                           at step j. Used for validation: compare
                           forecasts_history[j][0, i] against x_actual_today[j, i]
                           to compute per-bin per-stock forecast error.
    """
    ...
    for j in 0..k-1:
        ...
        all_forecasts = zeros(k_remaining, N)
...
function compute_portfolio_mape(actual, forecast, portfolio_weights=None):
    """
    Compute portfolio-level MAPE matching BDF 2008 Table 2 methodology.

    Aggregates per-stock turnover forecasts and actuals into a single
    portfolio-level series using portfolio weights, then computes MAPE
    on the aggregated series. This produces much lower MAPE values than
    per-stock averaging (~0.08 vs ~0.40) because idiosyncratic forecast
    errors diversify away across stocks.

    Parameters:
        actual:            array of shape (n_obs, N_stocks) — actual turnover
        forecast:          array of shape (n_obs, N_stocks) — forecast turnover
        portfolio_weights: array of shape (N_stocks,) — portfolio weights,
                           must sum to 1.0. If None, uses equal weights (1/N).

    Returns:
        portfolio_mape: float — MAPE of the portfolio-level aggregate
    """
    # Reference: BDF 2008, Section 4.3.3 — "the VWAP for the whole index
    # based on the average of VWAP over equities" using "the same weights
    # as used for construction of the index."
    #
    # BDF uses CAC40 index weights but does not provide them explicitly.
    # Equal weights (1/N) are a reasonable approximation for broad,
    # near-equal-weight indices; for cap-weighted indices, market-cap
    # weights should be used if available.
    # Researcher inference: equal-weight default for generality.

    N_stocks = actual.shape[1]

    if portfolio_weights is None:
        portfolio_weights = ones(N_stocks) / N_stocks

    # Aggregate turnover: weighted average across stocks per bin
    actual_portfolio = actual @ portfolio_weights       # shape (n_obs,)
    forecast_portfolio = forecast @ portfolio_weights   # shape (n_obs,)

    # MAPE on aggregate series
    mask = (actual_portfolio > 0)
    portfolio_mape = mean(
        abs(actual_portfolio[mask] - forecast_portfolio[mask])
        / actual_portfolio[mask]
    )
    return portfolio_mape
```
...
Output: weights_history — list of k arrays, weights_history[j] has shape (k-j, N)
        forecasts_history — list of k arrays, forecasts_history[j] has shape (k-j, N)
        (forecasts_history[j][0, i] is the one-step-ahead forecast for stock i at step j)

Validation: compute_mape(actual, forecast) — per-stock per-bin MAPE (Szucs 2017)
            compute_mse_star(actual, forecast, avg) — scale-adjusted MSE (Szucs 2017)
            compute_portfolio_mape(actual, forecast, weights) — portfolio MAPE (BDF 2008)
```
...
**NOTE ON VWAP EXECUTION COST VALIDATION**: The VWAP execution cost is defined as
MAPE of (realized_execution_price - VWAP) / VWAP (BDF 2008, Section 4.3.1). This
requires trade-level price data for each intraday bin, which is beyond the scope
of this volume model. These benchmarks are provided for context only — they
demonstrate the downstream benefit of better volume forecasts when paired with an
execution engine. The primary validation targets for this implementation are volume
forecast accuracy metrics: per-stock MAPE (via `compute_mape`), MSE* (via
`compute_mse_star`), and portfolio-level MAPE (via `compute_portfolio_mape`). To
reproduce the VWAP execution cost benchmarks, the developer would need to: (a)
obtain intraday VWAP prices per bin, (b) simulate trade execution using the model's
weights, and (c) compute the execution price vs. daily VWAP spread.
(Researcher inference: scoping note for implementability.)
...
| Portfolio-level MAPE computation | BDF 2008, Section 4.3.3; researcher inference for equal-weight default |
| VWAP execution cost benchmarks | BDF 2008, Tables 2, 4-7 |
| VWAP cost validation scope note | Researcher inference (scoping for implementability) |
