...
FUNCTION estimate_model(turnover_history, n_stocks, k_bins, L_days):
    ...
    FOR i = 1 TO N:
        ...
        //
        // Grid search range: use the 15th to 85th percentile of the
        // lagged values {e_{i,1}, ..., e_{i,T-1}} (the regressor values,
        // not the full series) as candidate thresholds. In practice the
        // distinction is negligible for T >= 500 since the two sets differ
        // by one observation at each end. Use 1-percentile steps (71
        // candidates). If either regime has fewer than max(15, 0.10 * T)
        // observations, skip that candidate.
...
FUNCTION forecast_intraday(observed_turnover_today, j_current,
    ...
    FOR i = 1 TO N:
        // Check for pathological negative forecast concentration
        // (Researcher inference: fallback to prevent volume share distortion)
        n_remaining = k_bins - j_current
        n_negative = count(c_forecast[i, j] + e_forecast[i, j] < 0
                           for j in j_current+1..k_bins)
        IF n_negative > 0.5 * n_remaining:
            // Too many negative forecasts -- specific component is unreliable
            // Fall back to common component only for all remaining bins
...
**Model selection across stocks:** The primary implementation should use SETAR
uniformly for all stocks in the cross-section, not select per stock. BDF 2008's
results show SETAR wins for the large majority of stocks (only 3 of 31 favor
AR), and the marginal computational cost of fitting SETAR vs AR(1) is small
(only the threshold grid search is extra, and it runs in milliseconds per stock).
If per-stock model selection is desired as an extension, compare in-sample AIC
between AR(1) and SETAR for each stock and select the model with lower AIC.
However, this adds complexity with minimal expected benefit given SETAR's broad
dominance.
(Reference: BDF 2008, Section 3.2; Researcher inference for the AIC-based
per-stock selection alternative)
...
4. **SETAR initialization:** BDF 2008 specifies MLE but does not detail the
   ...
   - Compute candidate thresholds: percentiles of the lagged values
     {e_{i,1}, ..., e_{i,T-1}} from the 15th to 85th percentile in
     1-percentile steps (71 candidates). The grid is computed from the
     regressor values (which differ from the full series by one observation
     at each end; negligible for T >= 500).
...
**Factor sign indeterminacy across daily re-estimations:** When the PCA is
re-estimated on a shifted rolling window, the sign of the eigenvectors (and
hence the factors F and loadings A) may flip from one day to the next. This
does not affect the common component (C = F @ A, so simultaneous sign flips
in F and A cancel out) or any downstream forecast. However, it can confuse a
developer who visualizes or logs the factor values and sees them flip sign
across consecutive days. If consistent factor sign is desired for visualization
or interpretability, apply a sign convention after each estimation: e.g.,
require that the first element of each loading vector a_j be positive (if
A[j, 0] < 0, negate both F[:, j] and A[j, :]).
(Researcher inference; standard PCA sign convention practice)
...
3. **Forecast accuracy (MAPE):** For liquid U.S. stocks at 15-minute resolution:
   ...
   Note on MAPE scale discrepancy: The ~5x difference in MAPE between the two
   papers (0.07-0.09 vs 0.40-0.50) is explained by the level of aggregation:
   - BDF 2008 appears to compute MAPE at the portfolio level, averaging
     turnover predictions across all N stocks first and then computing
     percentage errors on the smoother portfolio-level series. Portfolio
     averaging diversifies away stock-level noise, dramatically reducing MAPE.
   - Szucs 2017 (Eq. 2, p.6) computes per-stock per-bin MAPE and then
     averages across stocks. Individual stock turnover in 15-minute bins is
     much noisier than portfolio-level turnover, yielding higher MAPE.
   Additionally, the datasets differ: CAC40 European stocks (2003-2004) vs
   DJIA US stocks (2001-2012), different bin widths (20-min vs 15-min), and
   different sample lengths (1 year vs 11 years).
   When validating, compute per-stock per-bin MAPE following Szucs 2017 Eq.
   (2); expect values around 0.40 for liquid US equities. The BDF 2008 values
   around 0.07-0.09 use a different aggregation level and should not be used
   as targets for per-stock validation. Both papers show the same relative
   ordering: BDF-SETAR > BDF-AR(1) > U-method.
   (Researcher inference for the aggregation explanation; Szucs 2017 Eq. 2
   for per-stock MAPE definition)
...
2. **Negative turnover forecasts:** The additive structure means x_hat = c + e
   ...
   **Volume share impact:** If many remaining bins produce negative forecasts
   before flooring, the few bins with positive forecasts will receive
   disproportionately large volume shares, potentially concentrating all
   remaining volume in a single bin. To prevent this pathological behavior:
   if more than 50% of remaining bins (j_current+1 through k_bins) produce
   negative raw forecasts (before flooring), fall back to the common component
   forecast alone for all remaining bins (set e_forecast = 0). This ensures
   volume shares remain reasonable even when the specific component model
   produces erratic multi-step forecasts.
...
| Negative forecast handling and volume share fallback | Researcher inference | -- |
| MAPE scale discrepancy explanation (aggregation level) | Researcher inference | Szucs 2017 Eq. 2 for per-stock MAPE definition |
| Per-stock model selection guidance | Researcher inference | BDF 2008 Section 3.2 for SETAR dominance |
| Factor sign indeterminacy across re-estimations | Researcher inference | Standard PCA sign convention |
...
### Draft 3 Changes (in response to Critique 2)

Critique 2 found 0 major and 5 minor issues. All addressed below.

- **m1 (MAPE scale discrepancy):** Replaced the vague "different MAPE
  definitions" note in Expected Behavior with a specific explanation
  distinguishing portfolio-level MAPE (BDF 2008) from per-stock per-bin MAPE
  (Szucs 2017 Eq. 2). Added developer guidance: use per-stock MAPE (~0.40)
  as the validation target, not portfolio-level (~0.07-0.09).

- **m2 (Per-stock model selection):** Added "Model selection across stocks"
  paragraph in Variants section clarifying that SETAR should be used uniformly
  for all stocks. Noted optional AIC-based per-stock selection as extension
  with minimal expected benefit.

- **m3 (SETAR grid on lagged values):** Updated both Phase 1 Step 6 Option B
  and Initialization point 4 to specify the grid is computed from lagged
  values {e_{i,1}, ..., e_{i,T-1}} (the regressors), not the full series.
  Noted the distinction is negligible for T >= 500.

- **m4 (Negative forecast fallback):** Added fallback logic in Phase 2 Step 5:
  if >50% of remaining bins produce negative raw forecasts, reset e_forecast
  to 0 for all remaining bins (use common component only). Updated Edge Case
  #2 with explanation of the volume share concentration risk and the fallback
  rule.

- **m5 (Factor sign indeterminacy):** Added "Factor sign indeterminacy across
  daily re-estimations" paragraph in Calibration section explaining that sign
  flips are expected and do not affect forecasts, with optional sign convention
  (require first loading element to be positive) for visualization.
