...
function fit_ar1(e_series):
    ...
    params: dict with keys:
        ...
        sigma2: float -- innovation variance (unbiased estimate)
    """
...
**Note on overnight gaps:** The AR(1) model treats the series as contiguous across day
boundaries: the last bin of day d is followed by the first bin of day d+1. This introduces
periodic autocorrelation at lag k (visible in BDF 2008, Fig. 2: ACF of the specific
component shows spikes at multiples of 25). BDF and Szucs do NOT address this periodic
structure. The simple AR(1) captures only lag-1 dependence. Do not add seasonal AR terms
unless extending beyond the published model. (Researcher inference based on BDF 2008,
Fig. 2.)
...
function fit_setar(e_series, n_grid=100):
    ...
    # --- Phase 1: Grid search over tau to minimize total SSR ---
    ...
    # --- Phase 2: Final estimation for the winning tau ---
    # Re-estimate with the selected threshold and compute variance estimates.
    mask1 = (x_lag <= best_tau)
    mask2 = ~mask1
...
**Two-phase estimation:** The grid search (Phase 1) computes only SSR for each tau
candidate. Variance estimates (Phase 2) are deferred to the winning tau only. This avoids
unnecessary computation for the n_grid - 1 losing candidates. (Researcher inference:
...
```
function run_dynamic_execution(model, stock_idx, order_shares, observed_turnover):
    """
    Parameters
    ----------
    ...
    observed_turnover: list of float -- grows as bins complete; initially empty
    ...
    # Initialize e_{t-1} from the last in-sample observation.
    # This is the specific component residual from the final bin of the final day
    # in the estimation window. Using this value is correct: it is the most recent
    # estimate of the stock's idiosyncratic deviation, and all estimation data
    # precedes the forecast day (no look-ahead).
    # BDF 2008, Section 2.3: forecasts conditioned on information available at end
    # of estimation window.
    ...
    for j in range(k):
        ...
        # Forecast all remaining bins j..k-1 using iterated AR/SETAR
        forecasts_remaining = []
        e_chain = last_specific
        for future_j in range(j, k):
            x_hat, e_chain = forecast_turnover(
                model, stock_idx, future_j, e_chain
            )
        ...
        # Execute: trade weight[0] fraction of remaining shares in bin j
        shares_this_bin = weights[0] * shares_remaining
        schedule.append((j, shares_this_bin))

        # After bin j completes, observe actual and update state
        if j < len(observed_turnover):
            # Extract actual specific component:
            # e_actual = actual_turnover - c_hat_forecast[j]
            # NOTE: the common forecast is NOT updated -- it stays fixed.
            actual_specific = (
                observed_turnover[j] - model["common_forecast"][j, stock_idx]
            )
...
function u_method_benchmark(all_turnover_data, day_index, stock_idx, k, L):
    ...
    for j in range(k):
        ...
        for l in range(L):
            row = (day_index - l) * k + j
            bin_values.append(all_turnover_data[row, stock_idx])
        forecast[j] = mean(bin_values)
...
Model dict (estimated nightly)
    |
    v
[run_dynamic_execution] -- intraday loop for each stock:
...
**Type summary:**
- Turnover values: float64 (typical range 1e-5 to 1e-1 for individual 15-20 min bins)
- Factor matrix F_hat: float64, shape (P, r), r typically 1-3
- Loading matrix Lambda: float64, shape (N, r)
- Common component C_hat: float64, shape (P, N), non-negative
...
**Per-stock volume prediction accuracy (one-step-ahead):**
...
9. **U-method pairwise (MAPE):** Szucs 2017, Table 2c: BDF_AR beats U-method 32/1 by MAPE;
   BDF_SETAR beats U-method 32/1 by MAPE.
...
| fit_setar: per-regime variance | Researcher inference | Standard OLS |
...
| run_dynamic_execution: static dominated | BDF 2008 | Section 4.2, page 1717 |
| run_dynamic_execution: initialization | BDF 2008 | Section 2.3 |
| run_dynamic_execution: loop order | Researcher inference | Execution timing |
