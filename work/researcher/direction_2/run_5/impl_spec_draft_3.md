...
for j in range(k):
    # Get execution decision for bin j (single call, no actual_turnover)
    ...
    # Update state directly for next bin (no second call needed)
    last_specific = actual - model["common_forecast"][j, stock_idx]
    shares_remaining -= shares_to_trade
```

This single-call pattern avoids a redundant second invocation of `execute_one_bin`.
The state update (`last_specific` and `shares_remaining`) is trivial and done inline.
The `actual_turnover` parameter of `execute_one_bin` is retained for convenience but
is not needed in this standard usage pattern.

**Note: last-bin guarantee.** For the last bin (j = k-1), only one forecast remains,
so `compute_vwap_weights` returns `[1.0]` and `shares_to_trade = shares_remaining`.
This guarantees the full order is completed by market close. (Researcher inference:
follows directly from the weight computation.)
...
function daily_update(all_turnover_data, day_index, k, L, model_type):
    ...
    day_index: int -- current day (0-indexed); we forecast for day_index + 1.
        Precondition: day_index >= L - 1 (at least L days of history must
        precede the forecast day). If violated, start_row becomes negative
        and NumPy will silently return incorrect slices.
    k: int -- bins per day
    ...
    """
    # Guard: require at least L days of history
    assert day_index >= L - 1, f"day_index={day_index} < L-1={L-1}: insufficient history"
...
1. For each forecast day d (d = L+1 to D, where D is the total number of days).
   **Indexing convention mapping:** This description uses 1-based day numbering. In
   terms of the `daily_update` function (0-indexed): `day_index` ranges from L-1 to
   D-2. The first call uses `day_index = L - 1`, which estimates on days 0..L-1 and
   forecasts day L (= day d=L+1 in 1-based numbering).
